...
public:
	...
	//if (nBegin>=0 and nEnd>nBegin),then download range between nBegin and nEnd
	//if(nBegin>=0 and nEnd<=nBegin),then download range between nBegin and realy end;
	void SetRangeByte(unsigned int nBegin,unsigned int nEnd);
	void GetRangeByte(unsigned int &nBegin,unsigned int &nEnd);
protected:
...
private:
	...
	unsigned int m_nRangeByteBegin;//download range byte begin
	unsigned int m_nRangeByteEnd;
};
