...
function p.RefreshByDataIdx(idx)
    ...
    g_Select_Profession = p.BtnTagList[idx][6];
    g_Select_LookFace   = p.BtnTagList[idx][7];
...
function p.OnUIEvent(uiNode, uiEventType, param)
	...
	if uiEventType == NUIEventType.TE_TOUCH_BTN_CLICK then
        if tag == ID_ROLE_START_GAME then
            --检查
            if nil == g_Select_Name then
                --报提示
                return true;
            end
            --服务器端消息触发的发送创建消息
            if g_Create_Role_Reason ~= 0 then
                g_Create_Role_Reason = 0;
                MsgLogin.SendCreateRoleReq();
        ...
        elseif tag == ID_ROLE_OLDUSER_LOGIN then
            --老玩家登陆
            g_Select_Name = nil;
            g_Select_Profession = nil;
            g_Select_LookFace   = nil;
    ...
    -- 用户按下键盘的返回键
        if tag == ID_ROLE_EDIT_NAME then
            local edit = ConverToEdit(uiNode);
            if CheckP(edit) then
                g_Select_Name = edit:GetText();
                LogInfo("eidt text [%s][%s]", edit:GetText(),g_Select_Name);
            end
        end
