	...
	if uiEventType == NUIEventType.TE_TOUCH_BTN_CLICK then
        ...
        for n=1, nServerNum do
            if p.ServerListTag[n][2] == tag then
                g_Select_ServerName=p.ServerListTag[n][4];
                g_Select_ServerIp=p.ServerListTag[n][5];
                g_Select_ServerPort=p.ServerListTag[n][6];
                g_Inter_Server=p.ServerListTag[n][7];
--LogInfo("Register g_Inter_Server =[%d]ServerName[%s]", g_Inter_Server,g_Select_ServerName);
                if g_Inter_Server == 2 and g_Select_Account == nil then
--LogInfo("Fast Register");
                    --外服快速注册帐号
                    FastRegister();
                    g_Select_Account = GetFastAccount();
                    g_Account_Pwd = GetFastPwd();
                end
                LoginGame.LoginGame();
            end
        end
