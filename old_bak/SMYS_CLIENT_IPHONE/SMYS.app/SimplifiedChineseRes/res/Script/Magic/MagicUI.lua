...
function p.refreshMatrixItem(matrix, view, viewIds)
	...
	if (matrix) then
		...
		local sName = MatrixConfigFunc.GetDataBaseS(id,DB_MATRIX_CONFIG.NAME);
		local sDes = MatrixConfigFunc.GetDataBaseS(id, DB_MATRIX_CONFIG.DESCRIPT);
		local sLevel = string.format(" %d级", MatrixConfigFunc.GetUpLevelN(level, DB_MATRIX_UP_LEVEL.LEVEL));
		local suLevel = string.format("%d级", MatrixConfigFunc.GetUpLevelN(level + 1, DB_MATRIX_UP_LEVEL.USER_LEVEL));
		...
		expV:SetText(sSopth);
		if (nSopth == 0  or uSop < nSopth) then
...
function p.refreshAttackItem(item, view, viewIds)
	...
	if (item) then
		...
		local sName = AttackConfigFunc.GetDataBaseS(id,DB_MARTIALTYPE.NAME);
		local sDes = AttackConfigFunc.GetDataBaseS(id, DB_MARTIALTYPE.DESCRIPT);
