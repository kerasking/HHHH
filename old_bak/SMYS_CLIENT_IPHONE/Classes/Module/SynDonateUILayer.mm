...
void DragBar::Initialization(CGRect rect, unsigned int slideWidth)
{
	...
	//this->SetBackgroundImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("slider.png"), rect.size.width, rect.size.height), true);
	...
	NDPicture *picBg = pool.AddPicture(GetImgPathNew("slider.png"), width, 0);
	
	NDPicture *slide = pool.AddPicture(GetImgPathNew("slider_block.png"));
...
void SynDonateUILayer::Initialization()
{
	...
	btn->SetImage(pool.AddPicture(GetImgPathNew("btn_confirm.png")), false, CGRectZero, true);
	...
	imgBg->SetPicture(pool.AddPicture(GetImgPathNew("cell_mask.png"), 440, 20), true);
	...
	imgBg = new NDUIImage;
	imgBg->Initialization();
	imgBg->SetPicture(pool.AddPicture(GetImgPathNew("attr_listitem_bg.png"), 440, 20), true);
	imgBg->SetFrameRect(CGRectMake(6, fStartY, 440, 20));
	this->AddChild(imgBg);
