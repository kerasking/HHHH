...
void NpcTaskLayer::Initialization(vector<pair<int, string> >& vTasks)
{
	...
	imgLeftBg->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("bag_left_bg.png"), false), true);
	...
	imgRightBg->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("bag_bag_bg.png"), false), true);
	...
	scrollLayer->SetBackgroundImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("attr_role_bg.png"), 194, 184), true);
...
void NpcNode::Initialization()
{
	...
	this->CloseFrame();
	this->SetBackgroundPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("role_bg.png")));
	this->SetFocusImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("role_bg_focus.png")));
	...
	if (!m_bNpcInit) {
		...
		for (NDMapMgr::VEC_NPC::const_iterator it = npcs.begin(); it != npcs.end(); it++) {
			...
			if ( (npcState & QUEST_CANNOT_ACCEPT) > 0)
			{
				...
				imgTask->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPath("task_state_1.png")), true);
			...
			else if ( (npcState & QUEST_CAN_ACCEPT) > 0) 
			{
				NDUIImage* imgTask = new NDUIImage;
				imgTask->Initialization();
				imgTask->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPath("task_state_2.png")), true);
			...
			else if ( (npcState & QUEST_NOT_FINISH) > 0)
			{
				NDUIImage* imgTask = new NDUIImage;
				imgTask->Initialization();
				imgTask->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPath("task_state_3.png")), true);
			...
			else if ( (npcState & QUEST_FINISH) > 0)
			{
				NDUIImage* imgTask = new NDUIImage;
				imgTask->Initialization();
				imgTask->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPath("task_state_4.png")), true);
				imgTask->SetFrameRect(CGRectMake(70, 30, 16, 17));
				npcNode->AddChild(imgTask);
			}
			if (it == npcs.begin()) {
...
void NpcListLayer::Initialization(PlayerNpcListScene* npcListScene)
{
	...
	m_btnTask->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("btn_0.png")), false, CGRectZero, true);
	m_btnTask->SetFontColor(ccc4(255, 255, 255, 255));
	m_btnTask->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathBattleUI("itemchildback.png"), false), false, CGRectZero, true);
	...
	m_btnDaoHang->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("btn_0.png")), false, CGRectZero, true);
	m_btnDaoHang->SetFontColor(ccc4(255, 255, 255, 255));
	m_btnDaoHang->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathBattleUI("itemchildback.png"), false), false, CGRectZero, true);
...
void PlayerListLayer::Initialization(PlayerNpcListScene* scene)
{
	...
	for (; it != roles.end(); it++)
	{
		...
		if (!role->IsInState(USERSTATE_STEALTH)) //非隐形状态
		{
			...
			imgLevel->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("level_bg.png")), true);
			...
			imgHpMp->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("hp_mp.png")), true);
