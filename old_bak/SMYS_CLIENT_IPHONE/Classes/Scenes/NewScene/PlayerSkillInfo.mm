...
void CActSkillInfoLayer::Initialization()
{
	...
	m_btnKaiQi->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("bag_btn_click.png")), false, CGRectZero, true);
...
void CActSkillInfoLayer::RefreshBattleSkill(int idBattleSkill, int nSlotCount)
{
	if (CActSkillLayer::BUTTON_LOCK == idBattleSkill) {
		...
		m_imgSkill->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("lockstate.png")), true);
...
void CActSkillLayer::Initialization(const SET_BATTLE_SKILL_LIST& battleSkills, CActSkillInfoLayer* skillInfoLayer, int nMaxSlot)
{
	...
	NDPicture* picSkillButtonBg = pool.AddPicture(GetImgPathNew("bag_bagitem.png"));
	NDPicture* picSkillButtonFocus = pool.AddPicture(GetImgPathNew("bag_bagitem_sel.png"));
...
void CActSkillLayer::SetButtonImage(NDUIButton* pBtn)
{
	...
	if (idSkill > 0) {
		...
		pPic->Initialization(GetImgPathNew("bag_bagitem.png"));
		pPic->SetGrayState(true);
		pBtn->SetImage(pPic, true, CGRectMake(4, 4, 34, 34), true);
		pBtn->EnalbeGray(true);
	}
	else if (BUTTON_LOCK == idSkill) {
		pBtn->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("lockstate.png")), true, CGRectMake(1, 1, 40, 40), true);
	}
...
NDUIButton* CActSkillLayer::CreatePageButton(const char* pszTitle)
{
	...
	pBtn->Initialization();
	pBtn->SetBackgroundPicture(pool.AddPicture(GetImgPathNew("bag_btn_normal.png"), true),
							   pool.AddPicture(GetImgPathNew("bag_btn_click.png"), true),
...
void LifeSkillLayer::Initialization(LifeSkillInfoLayer* skillInfoLayer)
{
	...
	m_btnLianYao->SetBackgroundPicture(pool.AddPicture(GetImgPathNew("lifeskill_btn_normal.png"), 112, 32), NULL, false, CGRectZero, true);
	m_btnLianYao->CloseFrame();
	m_btnLianYao->SetFocusImage(pool.AddPicture(GetImgPathNew("lifeskill_btn_focus.png"), 112, 32), false, CGRectZero, true);
	...
	m_btnBaoShi->SetBackgroundPicture(pool.AddPicture(GetImgPathNew("lifeskill_btn_normal.png"), 112, 32), NULL, false, CGRectZero, true);
	m_btnBaoShi->CloseFrame();
	m_btnBaoShi->SetFocusImage(pool.AddPicture(GetImgPathNew("lifeskill_btn_focus.png"), 112, 32), false, CGRectZero, true);
	...
	NDPicture* pic = pool.AddPicture(GetImgPathNew("newui_tab_selarrow.png"));
	...
	imgExp->SetPicture(pool.AddPicture(GetImgPathNew("redexp.png")), true);
...
void PlayerSkillInfo::Initialization()
{
	...
	imgBg->SetPicture(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("bag_left_bg.png")), true);
...
void UserStateInfoLayer::Initialization()
{
	...
	m_btnYiChu->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("bag_btn_click.png")), false, CGRectZero, true);
