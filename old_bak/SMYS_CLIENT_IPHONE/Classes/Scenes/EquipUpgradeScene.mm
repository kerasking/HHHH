		...
		if (item && helpitem) 
		{
			switch (m_iType) {
				...
				{ //提升
					...
					bao << (unsigned char)EQUIP_IM_QUALITY << int(item->iID) << int(helpitem->iID);
					SEND_DATA(bao);
					break;
				}
				case EQUIP_ENHANCE: 
				{ //强化
					ShowProgressBar;
					NDTransData bao(_MSG_EQUIPIMPROVE);
					bao << (unsigned char)EQUIP_IM_ENHANCE << int(item->iID) << int(helpitem->iID);
					SEND_DATA(bao);
					break;
				}
				case EQUIP_UPLEVEL:
				{
					ShowProgressBar;
					NDTransData bao(_MSG_EQUIPIMPROVE);
					bao << (unsigned char)EQUIP_IM_UPLEV << int(item->iID) << int(helpitem->iID);
					SEND_DATA(bao);
					break;
				}					
				default:
...
bool EquipUpgradeScene::checkIsEquip(Item* item)
{
	...
	if (item->isCanEnhance() && (Item::isWeapon(item->iItemType) || Item::isDefEquip(item->iItemType) || Item::isAccessories(item->iItemType))) { // 装备类和防具
		return true;
	} else {		
		return false;
	}
