 ...
 */
class CCEaseInOut : public CEaseRateAction
{
public:
	virtual void Update(fTime time);
...
public:
	/** Creates the action with the inner action and the rate parameter */
	static CCEaseInOut* ActionWithAction(CActionInterval* pAction, float fRate);
};
...
/** 
 ...
 由快至慢,心电图型动作
 */
class CCEaseElasticOut : public CEaseElastic
{
public:
	virtual void Update(fTime time);
	virtual CActionInterval* Reverse(void);
...
public:
	/** creates the action */
	static CCEaseElasticOut* ActionWithAction(CActionInterval *pAction);
	/** Creates the action with the inner action and the period in radians (default is 0.3) */
	static CCEaseElasticOut* ActionWithAction(CActionInterval *pAction, float fPeriod);
};
