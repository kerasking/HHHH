...
#ifndef AFX_MD5CHECKSUM_H__2BC7928E_4C15_11D3_B2EE_A4A60E20D2C3__INCLUDED_
...
namespace Encrypt 
{
	...
	public:
		...
		virtual string		EncryptEx(unsigned char* buf, int nLen)	=0;
	...
	class CMD5Checksum : public IEncryptor
	{
		// Interface of IEncryptor
	public:
		virtual ULONG		Release(void)							{ delete this; return 0; };
		virtual int			Encrypt(unsigned char* buf, int nLen);
		virtual string		EncryptEx(unsigned char* buf, int nLen)	{return "";}
		virtual int			Decrypt(unsigned char* buf, int nLen)	{ return 0; };
		virtual int			Rencrypt(unsigned char* buf, int nLen)	{ return 0; };
		virtual void		ChangeCode(unsigned long ulCode)		{};
		virtual void		ChangeCode(const char* pszKey)			{};
		virtual IEncryptor*	Duplicate(void)							{ return NULL; };
		virtual int			ShakeHand(char* pszBuf, int nLenvoid)	{ return 0; }
	...
	class CMD5ChecksumStandard : public CMD5Checksum
	{
	public:
		virtual int			Encrypt(unsigned char* buf, int nLen);
		virtual string		EncryptEx(unsigned char* buf, int nLen);
		string byteHEX(unsigned char byte0) ;
	};
