	...
	if (tab->IsKindOfClass(RUNTIME_CLASS(VerticalTabLayer))) {
		if (lastIndex == 0 && curIndex == 1) {
			...
			if (m_tabFunc) {
				...
				btnTask->SetBackgroundPicture(pool->AddPicture(NDPath::GetImgPathBattleUI("btn_bg0.png")), NULL, false, CGRectZero, true);
				btnTask->SetFontColor(ccc4(255, 255, 255, 255));
				btnTask->SetImage(NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPathBattleUI("task.png"), false), false, CGRectZero, true);
				//btnTask->SetTitle("任务");
				...
				btnDaoHang->SetBackgroundPicture(pool->AddPicture(NDPath::GetImgPathBattleUI("btn_bg0.png")), NULL, false, CGRectZero, true);
				btnDaoHang->SetImage(NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPathBattleUI("dao_hang.png"), false), false, CGRectZero, true);
...
bAddToLeft = !bAddToLeft; \
	...
	if (role) {
		...
		NDPicture* pic = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPathBattleUI("btn_bg0.png"));
		...
		btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("trade.png")), false, CGRectZero, true);
		...
		if (!player.isTeamMember())
		{
			if (role->isTeamMember())
			{
				...
				btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("join_team.png")), false, CGRectZero, true);
				//btn->SetTitle("进队");
			}
			else
			{
				fastinit(ePlayerInviteTeam);
				btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("invite_team.png")), false, CGRectZero, true);
				//btn->SetTitle("组队");
			}
		}
		...
		btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("add_friend.png")), false, CGRectZero, true);
		//btn->SetTitle("好友");
		fastinit(ePlayerPrivateTalk);
		btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("private_talk.png")), false, CGRectZero, true);
		//btn->SetTitle("私聊");
		
		if (!role->IsInState(USERSTATE_PVE))
		{
			fastinit(ePlayerPK);
			btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("PK.png")), false, CGRectZero, true);
			//btn->SetTitle("PK");
		}
		
		fastinit(ePlayerReherse);
		btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("reherse.png")), false, CGRectZero, true);
		//btn->SetTitle("比武");
		
		if (role->IsInState(USERSTATE_FIGHTING))
		{
			fastinit(ePlayerWatchBattle);
			btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("watch_battle.png")), false, CGRectZero, true);
			//btn->SetTitle("观战");
		}
		...
		if (player.level < 20 && role->level >= 20) 
		{
			...
			btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("baishi.png")), false, CGRectZero, true);
			//btn->SetTitle("拜师");
		} 
		else if (player.level >= 20 && role->level < 20)
		{
			fastinit(ePlayerShouTu);
			btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("shoutu.png")), false, CGRectZero, true);
			//btn->SetTitle("收徒");
		}
		
		fastinit(ePlayerMail);
		btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("mail.png")), false, CGRectZero, true);
		//btn->SetTitle("邮件");
		
		if (player.getSynRank() >= SYNRANK_MENZHU_SHENG) {
			fastinit(ePlayerInviteSyn);
			btn->SetImage(pool.AddPicture(NDPath::GetImgPathBattleUI("invite_syn.png")), false, CGRectZero, true);
			//btn->SetTitle("军团");
		}
