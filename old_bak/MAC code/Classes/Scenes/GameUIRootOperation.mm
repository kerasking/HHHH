...
void NDUIHControlContainer::SetButtonName(const char* name)
{
	...
	m_picBasic->Initialization(NDPath::GetImgPath(name));
...
void NDUIHControlContainer::SetBGImage(const char* name)
{
	if (m_hclBasic || name)
	{
		...
		pic->Initialization(NDPath::GetImgPath(name));
		...
		layer->SetBackgroundImage(NDPath::GetImgPath(name));
...
void NDUIAniLayer::Initialization(const char * aniname)
{
	...
	NDUILayer::Initialization();
	sprite = new NDSprite;
	sprite->Initialization(NDPath::GetAniPath(aniname));
	sprite->SetPosition(CGPointMake(0, 120));
	
	m_picBg = new NDPicture;
	m_picBg->Initialization(NDPath::GetImgPathNew("Pic_Messagebg.png"));
