...
#define bag_image ([[NSString stringWithFormat:@"%s", NDPath::GetImgPath("titles.png")] UTF8String])
...
void GamePlayerBagScene::InitEquipItemList(int iEquipPos, Item* item)
{
	...
	if (!btn)
	{
		...
		picBack = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPath("ui_item.png"));
	...
	if (item->isRidePet())
	{
		...
		if (sumRepair == 0) {
			...
			if (sumRepair > NDPlayer::defaultHero().money) {
				stringstream ss; ss << "修理该装备的费用是" << sumRepair << ",你身上的银币不够,无法修理该装备";
...
void GamePlayerBagScene::repairAllItem()
{
	...
	if (sumRepair == 0) {
		...
		if (sumRepair > NDPlayer::defaultHero().money) {
			stringstream ss; ss << "修理所有装备的费用是" << sumRepair << "你身上的银币不够,无法修理全部";
			showDialog("温馨提示", ss.str().c_str());
		} else {
			stringstream ss; ss << "修理身上所有装备物品需要 " << sumRepair << " 银币，是否修理全部？";
			NDPlayerBagDialog *dlg = new NDPlayerBagDialog;
