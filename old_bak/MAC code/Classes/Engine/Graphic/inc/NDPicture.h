...
namespace NDEngine
{
    ...
    public:	
         static UIImage* GetUIImageFromFile(const char* pszPngFile);
	public:	
	...
	class NDPicture : public NDTexture
	{
...
//		参数：imageFile本地图片资源
//		返回值：无
		void Initialization(const char* imageFile); override
...
//		返回值：本对象
	...
	public:	
		CCTexture2D *GetTexture(); override
		// 灰图功能的NDPicture禁用该接口
		void SetTexture(CCTexture2D* tex); override
	private:
		//CCTexture2D *m_texture;
		CGRect m_cutRect;
...
//		返回值：NDPicture对象指针
        ...
        CCTexture2D* AddTexture(const char* imageFile); 
//		
//		函数：RemovePicture
//		作用：释放本地图片资源
//		参数：imageFile本地图片资源
//		返回值：无
	...
	private:
		...
		std::map<CCTexture2D*, std::string> m_mapTex2Str;
		
		std::map<std::string, CGSize> m_mapStr2Size;
		
	private:
		CGSize GetImageSize(std::string filename);
		CGSize GetPngImageSize(std::string filename);
	};
}
