...
{
	NSAssert(imageName != nil, @"TextureCache: imageName MUST not be nill");
...
- (int) Statistics:(BOOL)bNotPrintLog
{
	...
	for( id key in keys ) 
	{
		CCTexture2D * value = [self textureForKey:key];
		unsigned int n		= (unsigned int)[value GetPixelMemory]; 
		nSize				+= n;
		if (!bNotPrintLog)
		{
			NSLog(@"\n[%@]retaincount[%d]size[%u] [%d] kbyte  [%d] MByte\n", key, [value retainCount], n, n / 1024, n / (1024 * 1024));
		}
	}
	if (!bNotPrintLog)
	{
		NSLog(@"CCTextureCache texture size [%d] kbyte [%d] MByte", nSize / 1024, nSize / (1024 * 1024));
	}
	return nSize;
}
@end
