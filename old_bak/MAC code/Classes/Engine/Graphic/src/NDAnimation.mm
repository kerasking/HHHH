...
{
    //本帖结束，下一帖是首帖
    if ([runFrameRecord isThisFrameEnd]) 
    {
        return runFrameRecord.currentFrameIndex == (runFrameRecord.totalFrame-1) && runFrameRecord.repeatTimes <= 1;
    }
...
{
    int framecnt = [_frames count];
	if (framecnt) 
	{		
		if (runFrameRecord.currentFrameIndex >= (int)framecnt) 
		{
			return;
		}
        ...
        runFrameRecord.totalFrame = (int)framecnt;
        runFrameRecord.enduration = frame.enduration;
		
		//判断是否允许跑下一帧，如果允许则跑下一帧，否则还是跑当前帧
		if ([runFrameRecord enableRunNextFrame]) 
		{
			//runFrameRecord.isCompleted = NO;	
			//取下一帧
			frame = [_frames objectAtIndex:runFrameRecord.nextFrameIndex];
			
			[runFrameRecord NextFrame:(int)framecnt];
            //判断播放次数
			if (runFrameRecord.currentFrameIndex + 1 == (int)framecnt){
                _m_nPlayCount++;
            }
			/*
			//当前帧的索引值改变
