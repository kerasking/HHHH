...
namespace NDEngine
{
	...
	bool NDTransData::encrypt(std::string key)
	{
		...
		if ( GetSize() == ND_C_HEAD_SIZE ) return true; // 没有消息体需要加密
		...
		if( !CDes::Encrypt_Pad_PKCS_7(
									key.c_str(),
									 ...
									 (unsigned int)(GetSize()-ND_C_HEAD_SIZE),
									 tmpBuf,
									 len ) 
		   )
		   return false;
	...
	bool NDTransData::decrypt(std::string key)
	{
		...
		if ( GetSize() == ND_C_HEAD_SIZE ) return true; // 没有消息体需要解密
    ...
    {
        NSString *nsstr = ReadUnicodeNString();
