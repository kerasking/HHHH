...
namespace NDEngine
{
	...
	void NDTouch::Initialization(UITouch *touch)
	{
		...
		{
			m_location = CGPointMake(NDDirector::DefaultDirector()->GetWinSize().width - m_location.y , m_location.x ); 
			m_previousLocation = CGPointMake(NDDirector::DefaultDirector()->GetWinSize().width - m_previousLocation.y , m_previousLocation.x);
		}
	}
