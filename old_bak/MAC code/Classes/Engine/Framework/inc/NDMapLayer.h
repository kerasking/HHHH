...
enum MAP_SWITCH_TYPE {
	SWITCH_NONE = 0,
	SWITCH_TO_BATTLE,			//切屏到黑屏，初始战斗场景
	SWITCH_BACK_FROM_BATTLE,
	SWITCH_START_BATTLE,		//黑屏到亮屏，开始战斗
};
...
namespace NDEngine 
{
	class NDMapLayer : public NDLayer,public ITimerCallback, public ISMUpdateEvent
	{
...
//		返回值：无
	...
	//		void setRoadBlock(int x,int y){roadBlockX=x;roadBlockY=y;}
        void SetBattleType(int iBattleType);      
	private:
		void MakeOrders();
	...
	private:
		...
		NSMutableArray *m_ordersOfMapscenesAndMapanimations;
		NDMapData *m_mapData;
