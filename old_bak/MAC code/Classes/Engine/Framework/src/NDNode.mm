...
namespace NDEngine
{
	...
	NDNode::~NDNode()
	{
		//不要在析构函数中调用虚函数 jhzheng
		NDAsssert(m_ccNode != NULL);
        if (!m_ccNode) {
            return;
        }
        int nSize = m_childrenList.size();
		for (int i = nSize-1; i >= 0; --i) {
			NDNode* node = (NDNode*)m_childrenList[i];
			if (!node) {
                continue;
            }
			LuaObject funcObj;
			if (node->GetDestroyNotify(funcObj)|| funcObj.IsFunction())
			{
				...
				luaDestroyEventCallBack(node, true);
			}
			CCNode *ccNode = node->m_ccNode;
			node->SetParent(NULL);
            [m_ccNode removeChild:ccNode cleanup:true];
		}
	...
	void NDNode::RemoveAllChildren(bool bCleanUp)
	{
		...
		std::vector<NDNode*>::iterator it = m_childrenList.begin();
		while (it != m_childrenList.end()) 
		{
			...
			if (!this->CanDestroyOnRemoveAllChildren(node))
			{
				it++;
				continue;
			}
	...
	void NDNode::RemoveChild(NDNode* node, bool bCleanUp)
	{
		...
		for ( ; iter != this->m_childrenList.end(); iter++) 
		{
			...
			if (ndNode && ndNode->m_ccNode == node->m_ccNode)
			{
				LuaObject funcObj;
	...
	bool NDNode::CanDestroyOnRemoveAllChildren(NDNode* pNode)
	{
		return true;
	}
