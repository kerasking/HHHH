...
//		m_titleTimerTag=-1;
	...
	void NDMapLayer::replaceMapData(int mapId,int center_x,int center_y)
	{
        ...
        //ScriptMgrObj.excuteLuaFunc("ShowLoading", "LoadingUI","正在为您加载资源....");
        this->DownLoadImgs();
        /*
		if (m_mapData) 
		{
        ...
        */
        ...
        //++Guosen 2012.7.5
        m_mapIndex = mapId;
        SetScreenCenter( CGPointMake(center_x,center_y) );
	}
    //下载缺少的图片
    void NDMapLayer::DownLoadImgs()
    {
    
        
         printf("downloadImage count[%d]", [m_mapData.needdownloadimgs count]);
        if ([m_mapData.needdownloadimgs count] < 1){
            if (m_mapData)
            {
                [m_mapData reload];
printf("MapSize Width [%d][%d] Heigth[%d][%d]",m_mapData.columns,m_mapData.unitSize,
       m_mapData.rows ,m_mapData.unitSize);
                this->SetContentSize(CGSizeMake((m_mapData.columns * m_mapData.unitSize)/*0.5f*SCREEN_SCALE*/, (m_mapData.rows * m_mapData.unitSize)/*0.5f*SCREEN_SCALE*/));
                this->MakeOrdersOfMapscenesAndMapanimations();
                this->MakeFrameRunRecords();
                CGSize winSize = NDDirector::DefaultDirector()->GetWinSize();
                m_screenCenter = ccp(winSize.width / 2, this->GetContentSize().height - winSize.height / 2);
        ...
        /*
        std::string strimgName = [[m_mapData.needdownloadimgs lastObject] UTF8String];
        std::string strUrl = ScriptMgrObj.excuteLuaFuncRetS("GetMapUrl","");
        strUrl += "/";
        strUrl += strimgName;
        std::string strPath = NDPath::GetMapResPath();
        [m_mapData.needdownloadimgs removeLastObject];
        //bool bInit = CSMUpdate::sharedInstance().Init(strUrl.c_str(),"dhsg",strPath.c_str(),this);
        //if(!bInit){
         //   return;
        //}
        
        /*
        CSMUpdate::sharedInstance().SetUpdateMode(CSMUpdate::UM_STATIC);
        CSMUpdate::sharedInstance().SetUpdatePackName(strimgName.c_str());
        printf("DownLoad From[%s] To[%s]", strUrl.c_str(), strPath.c_str());
        CSMUpdate::sharedInstance().Download(strUrl.c_str(),strPath.c_str(),false,this,false);
*/

    }
   void NDMapLayer::OnDownloadEvent(DWORD dwSizeFile,DWORD dwSideDownLoaded)
    {
    }
    void NDMapLayer::CompleteUpdate(ISMUpdateEvent::ERROR_CODE emErrCode)
    {
        if ([m_mapData.needdownloadimgs count] > 0){
            this->DownLoadImgs();
        }
    }
    void NDMapLayer::OnError(ISMUpdateEvent::ERROR_CODE emErrCode,const char* pszErrMsg)
    {
    }
	...
	void NDMapLayer::Initialization(const char* mapFile)
	{
		...
		m_switchAniGroup = [[NDAnimationGroupPool defaultPool] addObjectWithModelId:switch_ani_modelId];
	...
	{
        int x=NDDirector::DefaultDirector()->GetWinSize().width/2-75*SCREEN_SCALE;
        int y=30*SCREEN_SCALE;
		if(m_lbTitle&&m_lbTitleBg)
		{
			if(showTitle){
				if(titleAlpha<255){
                    if(m_lbTitle&&m_lbTitleBg){
                        //NDLog(@"x:%d,y:%d",x,y);
                        m_lbTitleBg->SetFrameRect(CGRectMake((NDDirector::DefaultDirector()->GetWinSize().width/2-105*SCREEN_SCALE), y, 210*SCREEN_SCALE, 30*SCREEN_SCALE));
                        //m_lbTitleBg->draw();
                        m_lbTitle->SetFrameRect(CGRectMake(x, y, 150*SCREEN_SCALE, 30*SCREEN_SCALE));
	...
	void NDMapLayer::ShowTreasureBox()
	{
		if(!m_TreasureBox)
		{
            ...
            m_TreasureBox->SetScale(0.5f*SCREEN_SCALE);
	...
	void NDMapLayer::ShowTitle(int title_id)
	{
		showTitle=true;
		titleAlpha=0;
		if(!m_lbTitle)
		{
			...
			NDPicture* picture= NDPicturePool::DefaultPool()->AddPicture([[NSString stringWithFormat:@"%@map_title/map_title_%d.png", IMAGE_PATH,title_id] UTF8String]);
		...
		if(!m_lbTitleBg)
		{
			...
			NDPicture* bg = NDPicturePool::DefaultPool()->AddPicture([[NSString stringWithFormat:@"%@map_title_back.png", IMAGE_PATH] UTF8String]);
	...
	void NDMapLayer::showSwitchSprite(MAP_SWITCH_TYPE type)
	{
		...
		switchSpriteNode->ChangeSprite([[NSString stringWithFormat:@"%@%@", aniPath, szAniFile] UTF8String]);//switchSpriteNode->ChangeSprite([[NSString stringWithFormat:@"%@scene_switch.spr", aniPath] UTF8String]);
		switchSpriteNode->SetFrameRect(CGRectMake(0,0,NDDirector::DefaultDirector()->GetWinSize().width,NDDirector::DefaultDirector()->GetWinSize().height));//++Guosen 2012.7.6
		switchSpriteNode->SetScale(2.0);//原 480×320 => 960×640
		this->GetParent()->AddChild(switchSpriteNode,ZORDER_MASK_ANI);
	}
	//++
	...
	{
        TICK_ANALYST(ANALYST_NDMapLayerdraw);
		...
		if (m_mapData && m_bNeedShow) 
		{
            ...
            TICK_ANALYST(ANALYST_NDMapLayerdraw1);
			if(switchSpriteNode)
			{
				if (switchSpriteNode->IsAnimationComplete())
				{
					...
					switch(switch_type){
						...
						case SWITCH_TO_BATTLE:
                        {
                            TICK_ANALYST(ANALYST_NDMapLayerdraw2);
            ...
            TICK_ANALYST(ANALYST_NDMapLayerdraw3);
			this->refreshTitle();
            TICK_ANALYST(ANALYST_NDMapLayerdraw4);
			this->RefreshBoxAnimation();
//			if(m_TreasureBox){
//				m_TreasureBox->RunAnimation(true);
//			}
            TICK_ANALYST(ANALYST_NDMapLayerdraw5);
            ...
            TICK_ANALYST(ANALYST_NDMapLayerdraw6);
			if (m_leRoadSign)
			{
...
//		DrawLine(ccp(ptDraw.x + m_ptCamarkSplit.x, ptDraw.y + 0), ccp(ptDraw.x + m_ptCamarkSplit.x, ptDraw.y + 320), ccc4(255, 0, 0, 255), 1);		
	...
	{	
        TICK_ANALYST(ANALYST_DrawScenesAndAnimations);
		this->MakeOrders();
        ...
        TICK_ANALYST(ANALYST_DrawScenesAndAnimations2);
		for (NSUInteger i = 0; i < orderCount; i++) 
		{
			...
			else if (index < sceneTileCount + aniGroupCount + switchCount)//切屏点
			{
				...
				if ([m_switchAniGroup.animations count] > 0) 
				{
					...
					if (this->isMapRectIntersectScreen([animation getRect])) 
					{					
						[animation runWithRunFrameRecord:frameRunRecord draw:YES scale:(0.5f*SCREEN_SCALE)];
						[mapSwitch draw];
					}
					else 
					{
	...
	void NDMapLayer::setStartRoadBlockTimer(int time,int x,int y)
	{
		...
		NDMapMgrObj.roadBlockX=x;
		NDMapMgrObj.roadBlockY=y;
	}
	...
	void NDMapLayer::OnTimer(OBJID tag)
	{
		if(tag==blockTimerTag)
		{
			...
			{
				NDMapMgrObj.roadBlockX=-1;
				NDMapMgrObj.roadBlockY=-1;
			...
			SAFE_DELETE(this->m_ndTitleTimer);
	...
	{
        TICK_ANALYST(ANALYST_MakeOrders);
        ...
        std::map<unsigned int, NDNode*> mapPosNode;
		for (int i = 0; i < (int)cld.size(); i++) 
		{
			NDNode *node = cld.at(i);
			//if (node->IsKindOfClass(RUNTIME_CLASS(NDManualRole))) 
			//{
            node->EnableDraw(true);
            if (node->GetPosx() == -1) {//未设置坐标的不参与遮挡处理
                continue;
            }
				//NDManualRole *role = (NDManualRole*)node;
				//role->EnableDraw(true);
				//int serverX = (node->GetPosx()-DISPLAY_POS_X_OFFSET)/m_mapData.unitSize;
				//int serverY = (node->GetPosy()-DISPLAY_POS_Y_OFFSET)/m_mapData.unitSize;
				//unsigned int key = (((unsigned int )(serverX)) << 16) + serverY;
                unsigned int key = (((unsigned int )(node->GetPosx())) << 16) + node->GetPosy();
				//int64_t key = role->GetServerX() << 32 + role->GetServerY();
				std::map<unsigned int, NDNode*>::iterator it = mapPosNode.find(key);
				if (it != mapPosNode.end()) 
				{
					NDNode *roleInMap = it->second;
                    //int nLevel = roleInMap->GetNodeLevel();
                    //int nnodeLevl=node->GetNodeLevel();
					if (roleInMap->GetNodeLevel() < node->GetNodeLevel() ) 
					{
                        //** chh 2012-08-27 **//
                        if (roleInMap->IsKindOfClass(RUNTIME_CLASS(NDPlayer)) == false) {
                            roleInMap->EnableDraw(false);
                        }
						
						mapPosNode[key] = node;
					}
					else if(roleInMap->GetNodeLevel() > node->GetNodeLevel())
					{
                        if (node->IsKindOfClass(RUNTIME_CLASS(NDPlayer)) == false) {
                            node->EnableDraw(false);
                        }
					}
				}
				else 
				{
					mapPosNode.insert(std::pair<unsigned int, NDNode*>(key, node));
				}
			//}
		}
	...
	void NDMapLayer::ShowRoadSign(bool bShow, int nX /*=0*/, int nY /*=0*/)
	{
		...
		if (!m_leRoadSign)
		{
			...
			m_leRoadSign->Initialization(NDPath::GetAniPath("click.spr"));
            m_leRoadSign->SetScale(0.5f*SCREEN_SCALE);
...
void NDMapLayer::SetBattleType(int iBattleType)
{
    m_iBttleType = iBattleType;
}
