...
{		
	...
	for ( int lay = 0; lay < _layerCount; lay++) 
		for (uint r = 0; r < _rows; r++) 
			...
			{					
                ...
                NDPicture* ptile_pic = NDPicturePool::DefaultPool()->AddPicture([imageName UTF8String]);
				//tile.texture	= [[CCTextureCache sharedTextureCache] addImage:imageName];
                tile.texture    = ptile_pic->GetTexture();
	...
	{		
		...
		tile.mapSize	= CGSizeMake(self.columns*TileWidth, self.rows*TileHeight);
		tile.cutRect	= CGRectMake(0, 0, picWidth, picHeight);
		tile.drawRect	= CGRectMake(x, y, winsize.width, winsize.height);
		tile.reverse	= reverse;	
    ...
    float fFlag = fScaleFactor < 2 ? 2 : 1;
	for (int i = 0; i < nodeCount; i++) 
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:imageName]) 
		{
            ...
            node.texture = ptile_pic->GetTexture();
            CGSize contentSize_ = [[node texture] contentSizeInPixels];
            contentSize_.width /= fFlag;
            contentSize_.height /=  fFlag;
            node.texture.pixelsWide /= fFlag;
            node.texture.pixelsHigh /= fFlag;
             node.texture.contentSizeInPixels = contentSize_;
    //         [[node texture] contentSizeInPixels] = contentSize_;
            
       //     [[node texture] SetNewSize: contentSize_];
            
     //       [node.texture size_] = contentSize_;
     //      [node.texture contentSizeInPixels : contentSize_];
      //      [[node texture] setContentSizeInPixels:contentSize_];     
