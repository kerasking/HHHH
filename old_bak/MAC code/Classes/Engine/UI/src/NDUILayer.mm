...
#import "NDTextNode.h"
#import "NDUtility.h"
#import "NDUIOptionButtonEx.h"
#import "NDUIDefaultTableLayer.h"
#import "NDTargetEvent.h"
#import "ScriptUI.h"
#import "UIChatText.h"
#include "NDScene.h"
...
namespace NDEngine
{
	...
	void NDUILayer::SetBackgroundImage(const char* imageFile)
	{
		...
		m_backgroudTexture = [[CCTexture2D alloc] initWithImage:[UIImage imageWithContentsOfFile:[NSString stringWithUTF8String:imageFile]]];
	}
	...
	void NDUILayer::draw()
	{	
        TICK_ANALYST(ANALYST_NDUILayer);
...
//					m_focusNode = (NDUINode*)this->GetChildren().at(0);
			...
			else
			{
				...
				if (m_backgroudTexture) 
				{
					...
					[m_backgroudTexture ndDrawInRect:scrRect];	
...
//		}
	...
	{	
		...
		for (int i = this->GetChildren().size() - 1; i >= 0; i--) 
		{
			...
			//touch event deal.....
			CGRect nodeFrame = uiNode->GetBoundRect();//uiNode->GetScreenRect();
			...
			if (CGRectContainsPoint(nodeFrame, beginTouch)) 
			{
                ...
                {
                    //ScriptMgrObj.excuteLuaFunc("PlayEffectSound", "Music",1);
                   // int nSoundEffectId = ((NDUIButton *)uiNode)->GetSoundEffect();
                   // ScriptMgrObj.excuteLuaFunc("PlayEffectSound", "Music",nSoundEffectId);
	...
	{	
		...
		for (int i = this->GetChildren().size() - 1; i >= 0; i--) 
		{
			...
			//touch event deal
			CGRect nodeFrame = uiNode->GetBoundRect();
			//nodeFrame = RectAdd(nodeFrame, 2);			
	...
	bool NDUILayer::DispatchLayerMoveEvent(CGPoint beginPoint, NDTouch *moveTouch)
	{
        ...
        if(m_bIsHVContainer){
            ...
            if (pNode->IsKindOfClass(RUNTIME_CLASS(CUIScrollViewContainerM))){
                ...
                if(m_bIsHVFirstTemp == 1){
                    svc->OnScrollViewMove(this, 0, horizontal);
                }else if(m_bIsHVFirstTemp == 2){
            ...
            if(m_bIsHVFirstTemp){
                ...
                m_bIsHVFirstTemp = false;
            }
            return true;
        }
        if (m_bHorizontal){ // 水平滚动
			if (horizontal > 0.0f)
