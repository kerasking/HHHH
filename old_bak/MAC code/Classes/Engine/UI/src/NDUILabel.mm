...
namespace NDEngine
{
	...
	NDUILabel::NDUILabel()
	{
		...
		m_needMakeCoo = false;
		m_needMakeVer = false;
		m_fontSize = 15 * NDDirector::DefaultDirector()->GetScaleFactor();
	...
	void NDUILabel::SetFontSize(unsigned int fontSize)
	{
		...
		m_fontSize = fontSize * NDDirector::DefaultDirector()->GetScaleFactor();	
	...
	void NDUILabel::MakeCoordinates()
	{
		if (m_texture) 
		{
			...
			m_cutRect = CGRectZero;
			m_cutRect.size.width = m_texture.contentSizeInPixels.width;
			m_cutRect.size.height = thisRect.size.height < m_texture.contentSizeInPixels.height ? thisRect.size.height: m_texture.contentSizeInPixels.height;
	...
	void NDUILabel::MakeVertices()
	{
		if (m_texture) 
		{
            ...
            else if (m_textAlignment == LabelTextAlignmentLeftCenter)
			{
				drawRect.origin.x = scrRect.origin.x;
			...
			else if (m_textAlignment == LabelTextAlignmentCenter)
			{
				...
				drawRect.origin.y = scrRect.origin.y + (scrRect.size.height - m_cutRect.size.height) / 2 -2 ;
			}
			else 
			{
			...
			if (m_hasFontBoderColor) 
			{
                ...
                m_verticesBoder[0] = drawRect.origin.x + sf;
                m_verticesBoder[1] = winSize.height - drawRect.origin.y - drawRect.size.height - sf;
                m_verticesBoder[2] = 0;
                m_verticesBoder[3] = drawRect.origin.x + drawRect.size.width + sf;
                m_verticesBoder[4] = m_verticesBoder[1];
                m_verticesBoder[5] = 0;
                m_verticesBoder[6] = m_verticesBoder[0];
                m_verticesBoder[7] = winSize.height - drawRect.origin.y - sf;
                m_verticesBoder[8] = 0;
                m_verticesBoder[9] = m_verticesBoder[3];
                m_verticesBoder[10] = m_verticesBoder[7];
                m_verticesBoder[11] = 0;	
                
                
                
				for (int i = 0; i < 12; i++) {
					//m_verticesBoder[i] = m_vertices[i]+((i%3 == 2) ? 0.0f : 1.0f*NDDirector::DefaultDirector()->GetScaleFactor());
                    //m_verticesBoder[i] = m_vertices[i]-1.0f*NDDirector::DefaultDirector()->GetScaleFactor();
                }
			}
		}
	}
	...
	void NDUILabel::draw()
	{
        TICK_ANALYST(ANALYST_NDUILabel);
		NDUINode::draw();
