...
#define left_top_image [NSString stringWithFormat:@"%s", NDPath::GetImgPath("w3.png")]
#define right_bottom_image [NSString stringWithFormat:@"%s", NDPath::GetImgPath("w4.png")] 
...
//#define back_button_down_interval 10
	...
	void NDUIMenuLayer::Initialization()
	{
		...
		m_picCancel = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPath("titles.png"));
		if (m_picCancel)
		{
	...
	void NDUIMenuLayer::ShowOkBtn()
	{
		if (!m_btnOk) {
			...
			m_picOk = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPath("titles.png"));
			if (m_picOk)
			{
	...
	void NDUIMenuLayer::MakeLeftTopTile()
	{
        ...
        NDPicture* prightpic = NDPicturePool::DefaultPool()->AddPicture([left_top_image UTF8String]);
		//tile.texture = [[CCTextureCache sharedTextureCache] addImage:left_top_image];
        tile.texture = prightpic->GetTexture();
	...
	void NDUIMenuLayer::MakeRightTopTile()
	{
		...
		NDTile* tile = [[NDTile alloc] init];
         NDPicture* prightpic = NDPicturePool::DefaultPool()->AddPicture([left_top_image UTF8String]);
		tile.texture = prightpic->GetTexture();//[[CCTextureCache sharedTextureCache] addImage:left_top_image];
        SAFE_DELETE(prightpic);
		tile.cutRect = CGRectMake(0, 0, tile.texture.maxS * tile.texture.pixelsWide, tile.texture.maxT * tile.texture.pixelsHigh);
	...
	void NDUIMenuLayer::MakeLeftBottomTile()
	{
		...
		NDTile* tile = [[NDTile alloc] init];
        NDPicture* prightpic = NDPicturePool::DefaultPool()->AddPicture([right_bottom_image UTF8String]);
		tile.texture = prightpic->GetTexture();//[[CCTextureCache sharedTextureCache] addImage:right_bottom_image];
        SAFE_DELETE(prightpic);
		tile.cutRect = CGRectMake(0, 0, tile.texture.maxS * tile.texture.pixelsWide, tile.texture.maxT * tile.texture.pixelsHigh);
