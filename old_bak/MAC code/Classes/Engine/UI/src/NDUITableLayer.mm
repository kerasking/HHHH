...
{
	#define FONT_SIZE 15
	#define TITLE_PICTURE [NSString stringWithFormat:@"%s", NDPath::GetImgPath("plusMinus.png")] 
	...
	void NDUISectionTitle::SetFontColor(ccColor4B fontColor)
	{
		m_lblText->SetFontColor(fontColor);
	}
	
	ccColor4B NDUISectionTitle::GetFontColor()
	{
	...
	{
        TICK_ANALYST(ANALYST_NDUISectionTitle);
		NDUINode::draw();
	...
	void NDUIVerticalScrollBar::Initialization()
	{
		...
		m_picUp = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPath("scroll_arrow.png"));
		m_picDown = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPath("scroll_arrow.png"));
