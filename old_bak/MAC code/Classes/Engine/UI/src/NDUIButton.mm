...
//		this->AddChild(m_title);
	...
	void NDUIButton::SetImage(NDPicture* pic, bool useCustomRect, CGRect customRect, bool clearPicOnFree)
	{
		if (m_clearUpPicOnFree) 
		{
			SAFE_DELETE(m_image);
    ...
    void NDUIButton::SetDisImage(NDPicture *pic, bool useCustomRect/*= false*/, CGRect customRect/*= CGRectZero*/, bool clearPicOnFree/* = false*/)
	{
		if (m_ClearDisImageOnFree && m_disImage)
			delete m_disImage;
		m_disImage				= pic;
	...
	{
        TICK_ANALYST(ANALYST_NDUIBUTTON);
		NDUINode::draw();
		
		if (this->IsVisibled()) 
		{
            //** chh 2012-07-31 取消按钮放大效果 **//
            //m_touched = false;
            //m_longTouched = false;
			...
			if (parentNode) 
			{
				if (parentNode-IsKindOfClass(RUNTIME_CLASS(NDUILayer))) 
				{
					...
					{
						if (!m_touched || NULL == m_touchDownImage && !m_bChecked) 
						{
							if (m_image) m_image->SetColor(m_normalImageColor);							
							else if (m_combinepicImg) m_combinepicImg->SetColor(ccc4(255, 255, 255, 255));
						}	
						
						if (!IsTabSel() && ((m_touchDownStatus != TouchDownImage) || !m_touched || NULL == m_touchDownImage) && !m_bChecked )
						{
							if (m_useCustomRect) 
							{
                                ...
                                //focus时，不显示normal img
                                if (uiLayer->GetFocus() == this || IsTabSel()){
                                    if(m_focusStatus == FocusImage && m_focusImage && m_bFocusEnable){
	...
	{ 
		...
		NDUIButtonDelegate* delegate = dynamic_cast<NDUIButtonDelegate*> (this->GetDelegate());
		if (delegate) 
		{
			if (m_touched) 
			{
	...
	{
		if (pNode == m_scrtTitle || pNode == m_title || 
			pNode == m_lbTitle1 || pNode == m_lbTitle2 )
		{
			return false;
		}
		return true;
	...
	bool NDUIButtonDelegate::OnButtonDragOut(NDUIButton* button, CGPoint beginTouch, CGPoint moveTouch, bool longTouch)
	{
		return false;
	}
	
	bool NDUIButtonDelegate::OnButtonDragOutComplete(NDUIButton* button, CGPoint endTouch, bool outOfRange)
	{
		return false;
	}
	
	bool NDUIButtonDelegate::OnButtonDragIn(NDUIButton* desButton, NDUINode *uiSrcNode, bool longTouch)
	{
		return false;
	}
	...
	void NDUIButton::SetBackgroundPictureLua(NDPicture *pic, NDPicture *touchPic/*= NULL*/)
	{
...
//		}
	...
	void NDUIButton::SetText(const char* text1, 
				 const char* text2,
	...
	void NDUIButton::SetTwoTitle()
	{
		...
		CGRect	rect  = this->GetFrameRect();
		CGSize	size1 = getStringSize(m_lbTitle1->GetText().c_str(), m_lbTitle1->GetFontSize()),
				size2 = getStringSize(m_lbTitle2->GetText().c_str(), m_lbTitle2->GetFontSize());
				
		int iStartX = (rect.size.width-size1.width-size2.width-m_uiTwoTitleInter)/2;
		if (iStartX < 0) 
		{
			iStartX = 0;
		}
