...
static const char* DISPLAY_RESOLUTION_STR[DISPLAY_RESOLUTION_END]	=
{
	"",
	"_960_640",
	"_1024_768",
...
CUIData::CUIData(void)
{
	...
	display_resolution			= DISPLAY_RESOLUTION_480_320;
}
...
{
	//获取控件图片信息
	m_info.strNormalFile	= ini.GetValue(szCtrlName,GetValKeyStr(NORMAL_FILE_KEY));
	m_info.strSelectedFile	= ini.GetValue(szCtrlName,GetValKeyStr(SELECTED_FILE_KEY));
	m_info.strDisableFile	= ini.GetValue(szCtrlName,GetValKeyStr(DISABLE_FILE_KEY));
	m_info.strFocusFile		= ini.GetValue(szCtrlName,GetValKeyStr(FOCUS_FILE_KEY));
	m_info.strBackFile		= ini.GetValue(szCtrlName,GetValKeyStr(BACK_FILE_KEY));
	...
	pPos= ini.GetValue( szCtrlName, GetValKeyStr(CTRL_POS_KEY) );
	...
	pPos= ini.GetValue( szCtrlName, GetValKeyStr(CTRL_ANCHORPOS_KEY) );
	if(!pPos)
		return false;
	sscanf( pPos, "%f %f", &m_info.CtrlAnchorPos.x, &m_info.CtrlAnchorPos.y );


	m_info.nCtrlWidth	= ini.GetValueI(szCtrlName,GetValKeyStr(CTRL_WIDTH_KEY));
	m_info.nCtrlHeight	= ini.GetValueI(szCtrlName,GetValKeyStr(CTRL_HEIGHT_KEY));



	//取UV 信息
	pPos = ini.GetValue( szCtrlName, GetValKeyStr(NORMAL_FILE_UV_KEY) );
	if(!pPos)
		return false;
	sscanf( pPos, "%d %d %d %d", &m_info.rectNormal.x, &m_info.rectNormal.y, &m_info.rectNormal.w, &m_info.rectNormal.h);

	pPos = ini.GetValue( szCtrlName, GetValKeyStr(SELECTED_FILE_UV_KEY) );
	if(!pPos)
		return false;
	sscanf( pPos, "%d %d %d %d", &m_info.rectSelected.x, &m_info.rectSelected.y, &m_info.rectSelected.w, &m_info.rectSelected.h);


	pPos = ini.GetValue( szCtrlName, GetValKeyStr(DISABLE_FILE_UV_KEY) );
	if(!pPos)
		return false;
	sscanf( pPos, "%d %d %d %d", &m_info.rectDisable.x, &m_info.rectDisable.y, &m_info.rectDisable.w, &m_info.rectDisable.h);


	pPos = ini.GetValue( szCtrlName, GetValKeyStr(FOCUS_FILE_UV_KEY) );
	if(!pPos)
		return false;
	sscanf( pPos, "%d %d %d %d", &m_info.rectFocus.x, &m_info.rectFocus.y, &m_info.rectFocus.w, &m_info.rectFocus.h);

	pPos = ini.GetValue( szCtrlName, GetValKeyStr(BACK_FILE_UV_KEY) );
	if(!pPos)
		return false;
	sscanf( pPos, "%d %d %d %d", &m_info.rectBack.x, &m_info.rectBack.y, &m_info.rectBack.w, &m_info.rectBack.h);

	//取文本信息
	m_info.strText			= ini.GetValue(szCtrlName,GetValKeyStr(CTRL_TEXT_KEY));
	m_info.strTextAlign		= ini.GetValue(szCtrlName,GetValKeyStr(CTRL_TEXTALIGN_KEY));	
	m_info.strTextTradition = ini.GetValue(szCtrlName,GetValKeyStr(CTRL_TEXTTRADITION));
	m_info.nTextFontSize	= ini.GetValueI(szCtrlName,GetValKeyStr(CTRL_TEXTFONTSIZE));
	m_info.nTextFontColor	= ini.GetValueI(szCtrlName,GetValKeyStr(CTRL_TEXTFONTCOLOR));
...
std::string CUIData::GetValKeyStr(const char* str)
{
	if (!str)
	{ 
		return "";
	}
	if (DISPLAY_RESOLUTION_BEGIN > display_resolution  
		|| DISPLAY_RESOLUTION_END <= display_resolution)
	{
		return std::string(str);
	}
	return std::string(str);// + DISPLAY_RESOLUTION_STR[display_resolution];
}
