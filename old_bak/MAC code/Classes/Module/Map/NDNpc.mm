...
lable->SetFontSize(NPC_NAME_FONT_SIZE); \
lable->SetRenderTimes(3); \
} \
...
//lable->SetFontSize((12*(NDDirector::DefaultDirector()->GetScaleFactor())));
...
void NDNpc::Initialization(int lookface, bool bFaceRigth/*=true*/)
{	
	sex = lookface / 100000000 % 10;
	model = lookface %1000;/// 1000 % 100;//++Guosen2012.6.17
	//lookface = 2000000;
	...
	if (lookface <= 0) 
		...
		sprFile = [NSString stringWithFormat:@"%@model_%d%s", [NSString stringWithUTF8String:NDPath::GetAnimationPath().c_str()], lookface%1000, ".spr"];//lookface/1000000, ".spr"];//++Guosen2012.6.17
...
void NDNpc::OnDrawEnd(bool bDraw)
{
	...
	CGSize size		= getStringSize(m_name.c_str(), NPC_NAME_FONT_SIZE*fScaleFactor);
	
	int showX = npcpos.x;
    //高度临时调整，后续应该修改为在缩放时进行数据处理，否则坐标外部需要处理HJQ
	int showY = npcpos.y - size.height - ((m_currentAnimation ? ([m_currentAnimation bottomY]-[m_currentAnimation y]): 0)*0.5f*fScaleFactor);
//    int y_ani = (m_currentAnimation ? ([m_currentAnimation bottomY]-[m_currentAnimation y]): 0);
...
void NDNpc::SetNpcState(NPC_STATE state)
{
	...
	{
		m_picState = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetSMImgPath("mark_submit.png"));
        
        
	} 
	else if ( (npcState & QUEST_NOT_FINISH) > 0)
	{
		m_picState = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetSMImgPath("mark_task_accepted.png"));
	} 
	else if ( (npcState & QUEST_FINISH) > 0)
	{
		m_picState = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetSMImgPath("mark_task_accept.png"));
	}
	else if ( (npcState & QUEST_FINISH_SUB) > 0)
	{
		m_picState = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetSMImgPath("mark_task_accept2.png"));
	}       
 	else if ( (npcState & QUEST_CAN_ACCEPT_SUB) > 0)
	{
    ...
    if (m_picState) {
        //根据分辨率进行缩放
        m_picState->SetScale(0.5f*NDDirector::DefaultDirector()->GetScaleFactor());
    }
}
...
void NDNpc::SetLable(LableType eLableType, int x, int y, std::string text, ccColor4B color1, ccColor4B color2)
{
	...
	float fScaleFactor	= NDDirector::DefaultDirector()->GetScaleFactor();
	CGSize size = getStringSize(text.c_str(), NPC_NAME_FONT_SIZE*fScaleFactor);
	lable[1]->SetFrameRect(CGRectMake(x-(size.width/2)+1, y+sizewin.height-sizemap.height, sizewin.width, 30 * fScaleFactor));
	lable[0]->SetFrameRect(CGRectMake(x-(size.width/2), y+sizewin.height-sizemap.height, sizewin.width, 30 * fScaleFactor));
