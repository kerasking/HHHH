...
//				this->Walk(ccp(usRecordX*16+DISPLAY_POS_X_OFFSET, usRecordY*16+DISPLAY_POS_Y_OFFSET), SpriteSpeedStep4);
	...
	void NDManualRole::Initialization(int lookface, bool bFaceRight/*=true*/)
	{
...
//		if (sex % 2 == SpriteSexMale) 
		int model_id=lookface%1000;///1000000;//++Guosen2012.6.17
		//	if (sex % 2 == SpriteSexMale) 
...
//			NDSprite::Initialization(MANUELROLE_HUMAN_FEMALE);
		...
		//int weapon_id=lookface%100;
		//if(weapon_id!=0)
		//{
		//	weapon_id=lookface/100000%10*100+weapon_id;
		//	NDLog(@"manualRole set weapon:%d",weapon_id);
		//	this->SetWeaponImage(weapon_id);
		//}
	}
	void NDManualRole::ReLoadLookface(int lookface)
	{
		...
		NSString* aniPath = [NSString stringWithUTF8String:NDPath::GetAnimationPath().c_str()];
		this->reloadAni([[NSString stringWithFormat:@"%@model_%d.spr",aniPath,model_id] UTF8String]);
		this->SetCurrentAnimation(MANUELROLE_STAND, m_faceRight);
...
//		}
	...
	void NDManualRole::WalkToPosition(const std::vector<CGPoint>& vec_toPos, SpriteSpeed speed, bool moveMap, bool mustArrive/*=false*/)
	{
		...
		if (isTeamLeader() || !isTeamMember()) 
		{
			...
			this->MoveToPosition(vec_toPos, IsInState(USERSTATE_SPEED_UP) ? SpriteSpeedStep4 : SpriteSpeedStep8, moveMap, ignoreMask, mustArrive);
			if (isTeamLeader()) 
			{
...
//		}
		...
		else
		{
			...
			{
				AnimationListObj.sitAction(this,m_faceRight);
				return;
			}
			if (m_pBattlePetShow)
			{// 溜宠站立
	...
	{
		return ridepet != NULL && m_isRide && !isTransformed();
	}
...
//		}
		...
		*/
		...
		if ( scene->IsKindOfClass(RUNTIME_CLASS(CSMGameScene) ) )
        {
...
//	}
	...
	void NDManualRole::UpdateState(int nState, bool bSet)
	{
		...
		{
			if (bSet) {
				al.sitAction(this,this->m_faceRight);
				if (!m_picVendor) {
	...
	void NDManualRole::SetState(int nState)
	{
		...
		bool bNewInPractise	= this->IsInState(USERSTATE_PRACTISE);
		if (!bOldInPractise && bNewInPractise)
		{
			this->AddSMEffect(ScriptMgrObj.excuteLuaFunc<std::string>("GetPractiseEffectPath", "PlayerFunc"), 
							  eSM_EFFECT_ALIGNMENT_BOTTOM, eSM_EFFECT_DRAW_ORDER_FRONT);
		}
		else if (bOldInPractise && !bNewInPractise)
		{
			this->RemoveSMEffect(ScriptMgrObj.excuteLuaFunc<std::string>("GetPractiseEffectPath", "PlayerFunc"));
		}
	}
...
#undef DrawLable
	...
	void NDManualRole::SetLable(LableType eLableType, int x, int y, std::string text, ccColor4B color1, ccColor4B color2)
	{
		if (!subnode) 
		{
			return;
		...
		*/
        ...
        NDAnimation *animate = [m_aniGroup.animations objectAtIndex:0];
...
//		strKey.Format("Peerage%d", nPeerage);
	...
	bool NDManualRole::AddSMEffect(std::string strEffectPath, int nSMEffectAlignment, int nDrawOrder)
	{
		...
		std::map<std::string, NDLightEffect*>& mapEffect	= m_arrMapSMEffect[nDrawOrder][nSMEffectAlignment];
	...
	bool NDManualRole::RemoveSMEffect(std::string strEffectPath)
	{
		...
		for (int i = eSM_EFFECT_DRAW_ORDER_BEGIN; i < eSM_EFFECT_DRAW_ORDER_END; i++) 
		{
			...
			{
				std::map<std::string, NDLightEffect*>& mapEffect	= m_arrMapSMEffect[i][j];
	...
	void NDManualRole::RunSMEffect(int nDrawOrder)
	{
		...
		for (int j = eSM_EFFECT_ALIGNMENT_BEGIN; j < eSM_EFFECT_ALIGNMENT_END; j++) 
		{
			std::map<std::string, NDLightEffect*>& mapEffect	= m_arrMapSMEffect[nDrawOrder][j];
			if (mapEffect.empty())
			{
				continue;
			}
	...
	bool NDManualRole::ChangeModelWithMount( int nRideStatus, int nMountType )
	{
		...
		if ( nRideStatus == 0 )
		{
			reloadAni( [[NSString stringWithFormat: @"%@model_%d.spr",aniPath,nPlayerID] UTF8String] );
		}
		else
		{
			reloadAni( [[NSString stringWithFormat: @"%@model_%d_%d.spr",aniPath,nPlayerID,nMountID] UTF8String] );
		}
