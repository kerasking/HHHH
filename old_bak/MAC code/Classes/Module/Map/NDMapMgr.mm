...
#import "SyndicateUILayer.h"
#include "NewPaiHangScene.h"
#import "SynInfoUILayer.h"
#import "SynMbrListUILayer.h"
#import "SynElectionUILayer.h"
...
//		pool.RegMsg(_MSG_BOSS_BATTLE_SELF,this);
	...
	void NDMapMgr::AddManualRole(int nID, NDManualRole* role)
	{
		...
		//			return;
		//		}
		//if (NDDirector::DefaultDirector()->GetRunningScene())
            
            
            
        NDLayer *layer = getMapLayerOfScene( NDDirector::DefaultDirector()->GetSceneByTag(SMGAMESCENE_TAG));
		//NDLayer *layer = getMapLayerOfScene( NDDirector::DefaultDirector()->GetRunningScene());
		if (!layer)
		{
			return;
	...
	void NDMapMgr::LoadSceneMonster()
	{
		...
		for(unsigned int i=0;i<idList.size();i++){
			...
			if(m_id==theId){
				...
				int rule_id=ScriptDBObj.GetN("mapzone",idList.at(i),DB_MAPZONE_GENERATE_RULE_ID);
...
#pragma mark todo 宠物属性更新
			...
			case _MSG_LETTER_REQUEST:
				//EmailListener::processMsg(msgID, *data);//--Guosen 2012.8.20
				break;
    ...
    void NDMapMgr::ProcessLoginSuc(NDTransData& data)
    {
...
//	}
	...
	void NDMapMgr::processVersionMsg(NDTransData& data)
	{
		...
		if(pScene)
		{
			return pScene->OnMsg_ClientVersion(data);
		}
	}
	...
	void NDMapMgr::AddSwitch()
	{
		...
		NDMapData* mapdata = nil;
		if (!scene 
			|| (layer = getMapLayerOfScene(scene)) == NULL
			|| (mapdata = layer->GetMapData()) == nil)
		{
			return;
		}
...
#pragma mark 交易消息处理
	...
	void NDMapMgr::processUserInfo(NDTransData* data, int len)
	{
		...
		int nMoney = 0; (*data) >> nMoney; // 银币 4个字节
		...
		int dwEMoney = 0; (*data) >> dwEMoney; // 金币 4个字节
		int storageMoney = 0; (*data) >> storageMoney; // 仓库银币4个字节
		int storageEMoney = 0; (*data) >> storageEMoney; // 仓库金币4个字节
...
//        
		...
		{ // 本地图处理
			...
			player.SetPositionEx(ccp(dwPortalX*MAP_UNITSIZE*MAP_SCALE+DISPLAY_POS_X_OFFSET*MAP_SCALE, dwPortalY*MAP_UNITSIZE*MAP_SCALE+DISPLAY_POS_Y_OFFSET*MAP_SCALE));
			player.SetServerPositon(dwPortalX, dwPortalY);
			player.stopMoving();
			
			layer->SetScreenCenter(ccp(dwPortalX*MAP_UNITSIZE*MAP_SCALE+DISPLAY_POS_X_OFFSET*MAP_SCALE, dwPortalY*MAP_UNITSIZE*MAP_SCALE+DISPLAY_POS_Y_OFFSET*MAP_SCALE));
			
			player.SetAction(false);
			
			CloseProgressBar;
			ScriptMgrObj.excuteLuaFunc("StopLoading", "LoadingUI");
			return;
		}
		*/
//		player.idCurMap = idMapDoc;
...
//		//NDLog(@"%d", layer->GetChildren().size()); 
//#ifndef DEBUG
//		if(SystemSetMgrObj.GetBoolean(SYSTEM_BG_MUSIC_KEY,true))
//		{
...
//			//ScriptMgrObj.excuteLuaFunc("showBossMapUI", "",0);
...
//#endif 
	...
	void NDMapMgr::processPlayer(NDTransData* data, int len)
	{
		...
		int dwPkPoint	= 0;	(*data) >> dwPkPoint;// pk值
		//int weaponTypeID=0; (*data) >> weaponTypeID;
		//int armorTypeID=0; (*data) >> armorTypeID;
		...
		//++Guosen 2012.7.15
		int tRank		= data->ReadByte();
		int nRideStatus	= data->ReadInt();
		int nMountType	= data->ReadInt();
        int nQuality	= data->ReadByte();
		//unsigned int nMountlookface = ScriptDBObj.GetN( "mount_model_config", nMountType, DB_MOUNT_MODEL_LOOKFACE );
		role->ChangeModelWithMount( nRideStatus, nMountType  );
		//++
		
        role->m_nQuality = nQuality;
		...
		{
			// todo设置玩家打坐状态
			role->SetCurrentAnimation(7, role->IsReverse());
		}
		else
		{
			// todo取消打坐状态
			if (role->AssuredRidePet()){
				role->SetCurrentAnimation(role->GetPetStandAction(), role->IsReverse());
			}else{
				role->SetCurrentAnimation(0, role->IsReverse());
			}
		}
		// 处理宠物
...
//					}
	...
	void NDMapMgr::setManualroleTeamID(int iRoleID, int iTeamID)
	{
		...
		{
			role->teamId = iTeamID;
            if (iTeamID == role->m_id) {
                role->SetNodeLevel(NODE_LEVEL_TEAM_LEADER);
            }else{
                role->SetNodeLevel(NODE_LEVEL_NONE);
            }
		}
	}
    ...
    void  NDMapMgr::ProcessTempCredential(NDTransData& data)
    {
