...
void NDBaseRole::setMoveActionWithRidePet()
{
	...
	if ( m_faceRight == FACE_LEFT) 
	{
		this->SetCurrentAnimation(this->pet_run_action, false);
	} 
	else if (m_faceRight == FACE_RIGHT) 
	{
		this->SetCurrentAnimation(this->pet_run_action, true);
	}
...
void NDBaseRole::setStandActionWithRidePet()
{
	...
	if ( m_faceRight == FACE_LEFT) 
	{
		this->SetCurrentAnimation(this->pet_stand_action, false);
	} 
	else if (m_faceRight == FACE_RIGHT) 
	{
		this->SetCurrentAnimation(this->pet_stand_action, true);
	}
...
void NDBaseRole::SetEquipment(int equipmentId, int quality)
{
	...
	if (equipmentId >= 200 && equipmentId < 10000) //武器
	{
		...
		{
			if (this->GetRightHandWeaponImage() == nil) 
			{
...
{
	SAFE_DELETE_NODE(ridepet);
	this->pet_stand_action=stand_action;
	this->pet_run_action=run_action;
	this->pet_lookface=lookface;
	acc_level=acc;
	if (lookface!=0){
		ridepet = new NDRidePet;
		ridepet->Initialization(lookface);
	//ridepet->quality = quality;
		ridepet->SetPositionEx(this->GetPosition());
		this->m_isRide=true;
	//if (this->IsKindOfClass(RUNTIME_CLASS(NDManualRole))) 
