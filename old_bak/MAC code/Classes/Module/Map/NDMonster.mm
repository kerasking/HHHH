...
#define MONSTER_NAME_SIZE (12)
	...
	state(0)
	{
		...
		deadTime = [NSDate timeIntervalSinceReferenceDate];
		
		frozenTime = [NSDate timeIntervalSinceReferenceDate];
...
//		}
	...
	void NDMonster::SetMoveRect(CGPoint point,int size){
		...
		if (layer) {
			...
			if (mapdata){
				self_move_rectH = mapdata.rows*MAP_UNITSIZE;
			}
		}
...
//		} else {
	...
	bool NDMonster::OnDrawBegin(bool bDraw)
	{
		...
		if (!this->IsKindOfClass(RUNTIME_CLASS(NDBattlePet)) 
			&& this->GetParent()
...
//		}
	...
	void NDMonster::Update(unsigned long ulDiff)
	{
		...
		if (isFrozen) 
		{
			if ([NSDate timeIntervalSinceReferenceDate] - frozenTime >= FROZEN_TIME) 
			{
...
//					}
	...
	int  NDMonster::getRandomAIDirect()
	{
		...
		{
			if ( ![mapdata canPassByRow:monsterRow andColumn:monsterCol + 1 ] ) 
			{ // 该方向不能走
				xDirect = dir_invalid;
			} else {
				xDirect = dir_right;
		...
		{
			if ( ![mapdata canPassByRow:monsterRow + 1 andColumn:monsterCol ] )
			{// 该方向不能走
				yDirect = dir_invalid;
			} else 
			{
...
//		}
	...
	void NDMonster::directLogic()
	{
		...
		switch (moveDirect) 
		{
			case dir_up: 
			{ // 上
				if ( (y - EVERY_MOVE_DISTANCE) >= selfMoveRectY && [mapdata canPassByRow:row - 1 andColumn:col ] )
				{
			...
			case dir_down: 
			{ // 下
				if ((y + BASE_BOTTOM_WH + EVERY_MOVE_DISTANCE) <= selfMoveRectY + self_move_rectH && [mapdata canPassByRow:row + 1 andColumn:col ] ) 
				{
					pos.y += EVERY_MOVE_DISTANCE;
			...
			case dir_left: 
			{ // 左
				if ( (x - EVERY_MOVE_DISTANCE) >= selfMoveRectX && [mapdata canPassByRow:row andColumn:col - 1 ] ) 
				{
			...
			case dir_right:
			{ // 右
				if ( (x + BASE_BOTTOM_WH + EVERY_MOVE_DISTANCE) <= selfMoveRectX + self_move_rectw && [mapdata canPassByRow:row andColumn:col + 1 ] )
				{
					pos.x += EVERY_MOVE_DISTANCE;
	...
	void NDMonster::doMonsterCollides()
	{
		...
		if (monsterCatogary == MONSTER_BOSS 
			&& [NSDate timeIntervalSinceReferenceDate] - m_timeBossProtect < BOSS_PROTECT_TIME) 
		{
			isMonsterCollide = false;
			return;
		}
	...
	void NDMonster::sendBattleAction()
	{
		...
		switch (monsterCatogary)
		{
			...
			case MONSTER_ELITE: 
			{
                ...
                //精英怪跟普通怪都发 BATTLE_ACT_CREATE
                bao << (unsigned char)BATTLE_ACT_CREATE; 
				bao << (unsigned char)0; // btturn
				bao << (unsigned char)1; // datacount
				bao << m_id; // 怪物类型Id
				break;
...
//			bossRing->SetCurrentAnimation(0, false);
	...
	void NDMonster::endBattle()
	{
		if (monsterCatogary == MONSTER_BOSS) 
		{
			m_timeBossProtect = [NSDate timeIntervalSinceReferenceDate];
		}
	}
	...
	void NDMonster::drawName(bool bDraw)
	{
        ...
        /*
			...
			*/
			...
			CGSize size = getStringSize(m_name.c_str(), MONSTER_NAME_SIZE);
			CGSize sizemap;
			...
			if (!m_lbName) 
			{
				...
				m_lbName->SetText(NDCommonCString("aaaa"));//m_name.c_str());
			}
