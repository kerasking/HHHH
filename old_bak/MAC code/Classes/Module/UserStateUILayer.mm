...
void UserStateUILayer::processMsgUserState(NDTransData& data)
{
	...
	for (int i = 0; i < count; i++) {
		...
		state->idState = data.ReadInt();
		data.ReadInt();//idConfig
        ...
        //** chh 2012-6-16 SM中使用，当前我们的版本无用到如果以的有添加可将注释删除 **// 
		//state->shortTip = data.ReadUnicodeString();
		//state->descript = data.ReadUnicodeString();
        
        state->shortTip = "state->shortTip 测试";
		state->descript = "state->descript 测试";
