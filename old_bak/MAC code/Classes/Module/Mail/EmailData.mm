...
void EmailData::Init()
{
	...
	mGetMoney = 0; mGetEMoney = 0; // 邮件得到银币跟金币
	
	mPayMoney = 0; mPayEMoney = 0; // 收邮件附件所要支付的银币跟金币
 ...
 */
 void EmailData::setMStateStr() {
	...
	if (((mAttachState & ATTACH_ITEM_ID) == ATTACH_ITEM_ID) //附件
		|| (mAttachState & ATTACH_MONEY) == ATTACH_MONEY  //银币
		|| (mAttachState & ATTACH_EMONEY) == ATTACH_EMONEY) { //金币
