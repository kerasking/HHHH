...
EmailSendScene::EmailSendScene()
{
	...
	customViewTip[eGiveEmoney] = "请输入要赠送的金币";
	customViewTip[eGiveMoney] = "请输入要赠送的银币";
	customViewTip[ePayEmoney] = "请输入要收费的金币";
	customViewTip[ePayMoney] = "请输入要收费的银币";
}
...
void EmailSendScene::OnButtonClick(NDUIButton* button)
{
	...
	else if (button == m_menulayerBG->GetOkBtn()) 
	{
		...
		std::string tip = "每封邮件需收取手续费100银币。是否确定要发送邮件给玩家 ";
...
bool EmailSendScene::OnCustomViewConfirm(NDUICustomView* customView)
{
	...
	else if (iTag == eGiveMoney) 
	{
		...
		{
			std::stringstream ss; ss << "你背包中的银币数仅有" << NDPlayer::defaultHero().money << ",请重新输入";
			customView->ShowAlert(ss.str().c_str());
			return false;
		}
   ...
   else if (iTag == eGiveEmoney) 
   {
	   ...
	   if (iAmount < 0) 
	   {
		   customView->ShowAlert("失败, 请填入正确的赠送金币");
		   return false;
	   }
	   
	   if (iAmount > NDPlayer::defaultHero().eMoney) 
	   {
		   std::stringstream ss; ss << "你背包中的金币数仅有" << NDPlayer::defaultHero().eMoney << ",请重新输入";
		   customView->ShowAlert(ss.str().c_str());
		   return false;
	   }
