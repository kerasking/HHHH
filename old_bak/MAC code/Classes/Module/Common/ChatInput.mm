...
void ChatInput::ShowExpressions()
{
	if (!m_expressionLayer) 
	{
		...
		NDPicture* picExpression = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPath("titles.png"));
...
void ChatInput::ShowItems()
{
	if (!m_itemLayer) 
	{
		...
		NDPicture* picItem = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPath("titles.png"));
