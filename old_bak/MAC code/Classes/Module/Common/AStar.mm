...
//----------------------------------------------------//
BOOL CAStar::FindPath( NDMapLayer* pGamemap, const CMyPos& posStart, const CMyPos& posTarget,unsigned long dwMaxSerachTime, bool mustArrive/*=false*/)
{ 
	...
	if( (posStart.x ==  posTarget.x) 
	 && (posStart.y ==  posTarget.y))
		return TRUE;
	...
	if(pNode)
	{
		...
		pNode->nG = 0;
		pNode->nH = this->GetHValue(m_posStart);
		pNode->nStep = 0;
		pNode->pParent = NULL;
		pNode->nChildNum = 0;
		pNode->nDir = 0;
	...
	double dTimeBeg = [NSDate timeIntervalSinceReferenceDate];
	
	unsigned int pathstep = 0;
	
	while(true)
	{
		double dTimePass = [NSDate timeIntervalSinceReferenceDate]-dTimeBeg;
		...
		NodeInfo* pCurrentNode = this->GetCurrentNode();
		...
		if((pCurrentNode->nX == m_posTarget.x) 
		&& (pCurrentNode->nY == m_posTarget.y))
		{
			///\终点放入close 表
			this->GetPath(pCurrentNode);
			break;
		}

		
		///\加入close表 
		this->AddToList(m_setClose,pCurrentNode);
		this->SearchChild(pCurrentNode);
...
//----------------------------------------------------//
int CAStar::GetHValue( const CMyPos& pos)
{
...
struct COMPARE_NODE 
: public binary_function <int, NodeInfo* , bool> 
{
	bool operator()(
		const pair<int, NodeInfo*>& _1, 
		const pair<int, NodeInfo*>& _2
		) const
	{
...
void CAStar::UpdateChildren(NodeInfo* pParent)
{
	...
	while ( !StackNode.empty() )
	{
		...
		{///\遍历所有子节点 并将他们都push到栈中检查后续子节点
			...
			if ( pKidParent->nG + 1 < pKidNode->nG )
			{///\如果路径更优则更新他们的G H
				pKidNode->nG = pKidParent->nG +1;
				pKidNode->nF = nG + pKidNode->nH;
...
void CAStar::SearchChild(NodeInfo* pParentNode)
{
	...
	*/
	...
	for(int i = 0; i < 8; ++i)
	{
		...
		if ( pCheck = this->CheckList(m_setClose,nNumber) ) //if没问题!
		{///\已经存在close表中
			pParentNode->pChildNode[pParentNode->nChildNum++] = pCheck;
			if ( nG < pCheck->nG )
			{///\更新 G H
