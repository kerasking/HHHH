...
{
    TICK_ANALYST(ANALYST_QuickTalkCell);
...
const NSUInteger MAX_CHAT_INPUT = 64;

@implementation ChatTextFieldDelegate
...
{
	if (nil != m_chatDelegate.tfChat.subviews) {
...
Battle::~Battle()
{
	...
	//SAFE_DELETE(m_picActionWordDef);
	//SAFE_DELETE(m_picActionWordDodge);
	//SAFE_DELETE(m_picActionWordFlee);
	//SAFE_DELETE(m_battleBg);
...
void Battle::ShowChatTextField(bool bShow)
{
	...
	//		}
	//		
	//		if (nil == [m_chatDelegate.tfChat superview]) {
...
bool Battle::TouchEnd(NDTouch* touch)
{
	...
	if(f)
	{
        ...
        ScriptMgrObj.excuteLuaFunc<bool>( "SetFighterInfo", "FighterInfo", f->GetRole()->m_name, skillName, f->m_info.level );
...
void Battle::dealWithCommand()
{
	if (m_vCmdList.size() > 0 && m_battleStatus != BS_SHOW_FIGHT)
	{
		...
		for (UInt32 i = 0; i < this->m_vCmdList.size(); i++)
		{
			NDLog(@"%d action",i);
				...
				while (nextCmd) {
					...
					switch(nextCmd->btEffectType){
						...
						case BATTLE_EFFECT_TYPE_DRITICAL:
                            ...
                            action->isCriticalHurt	= true;//改为受击方显示伤害数字，而不是上面的显示暴击光效
		...
		switch (fa->effect_type) {
			...
			case BATTLE_EFFECT_TYPE_STATUS_LIFE://状态去血
				...
				if (fa->m_Actor->m_info.nLife > 0) {// hurt
					...
					NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath(ss.str().c_str())];
					NDAnimationGroup* dieAniGroup = [[NDAnimationGroup alloc] initWithSprFile:file];
					addSkillEffectToFighter(fa->m_Actor,dieAniGroup,0,1);
					[dieAniGroup release];
			...
			case BATTLE_EFFECT_TYPE_STATUS_MANA://状态去气
				fa->m_Actor->setCurrentMP((fa->m_Actor->m_info.nMana)+(fa->data));
                fa->action_status = ACTION_STATUS_FINISH;
				break;
			case BATTLE_EFFECT_TYPE_STATUS_ADD:

				{
					unsigned int	nIconID = ScriptDBObj.GetN( "skill_result_cfg", fa->data, DB_SKILL_RESULT_CFG_ICON );
                    unsigned int	effectId = ScriptDBObj.GetN( "skill_result_cfg", fa->data, DB_SKILL_RESULT_CFG_LOOKFACE );
                    
                    
					fa->m_Actor->AppendStatusIcon( nIconID );
            	    fa->action_status = ACTION_STATUS_FINISH;
			...
			case BATTLE_EFFECT_TYPE_STATUS_LOST:
				{
					unsigned int	nIconID = ScriptDBObj.GetN( "skill_result_cfg", fa->data, DB_SKILL_RESULT_CFG_ICON );
					fa->m_Actor->RemoveStatusIcon( nIconID);
    	            fa->action_status = ACTION_STATUS_FINISH;
    	        }
				break;
			case BATTLE_EFFECT_TYPE_ESCORTING://护驾=援护
                {//播放“援护”文字动画++Guosen 2012.7.9//
                    NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath("sm_effect_47.spr")];
                    NDAnimationGroup* effect = [[NDAnimationGroup alloc] initWithSprFile:file];
					addSkillEffectToFighter(fa->m_Actor,effect,0,g_ArrayEfectProp[47].iPos,g_ArrayEfectProp[47].bRevers);
                    [effect release];
                }
                fa->action_status = ACTION_STATUS_FINISH;
				break;
			case BATTLE_EFFECT_TYPE_COOPRATION_HIT://合击
                {//播放“合击”文字动画++Guosen 2012.7.9
                    NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath("sm_effect_46.spr")];
                    NDAnimationGroup* effect = [[NDAnimationGroup alloc] initWithSprFile:file];
					addSkillEffectToFighter(fa->m_Actor,effect,0,g_ArrayEfectProp[46].iPos,g_ArrayEfectProp[46].bRevers);
                    [effect release];
                }
                fa->action_status = ACTION_STATUS_FINISH;
				break;
			case BATTLE_EFFECT_TYPE_RESIST://免疫
                {//播放“免疫”文字动画++Guosen 2012.8.2
                    NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath("sm_effect_32.spr")];
                    NDAnimationGroup* effect = [[NDAnimationGroup alloc] initWithSprFile:file];
					addSkillEffectToFighter(fa->m_Actor,effect,0,g_ArrayEfectProp[32].iPos,g_ArrayEfectProp[32].bRevers);
                    [effect release];
                }
                fa->action_status = ACTION_STATUS_FINISH;
				break;
			case BATTLE_EFFECT_TYPE_CHANGE_POSTION://移位
				{//++Guosen 2012.7.10
					int targetX = countX( this->m_teamAmout, fa->m_Actor->m_info.group, (fa->team_def-1)%3+1, fa->data );
					int targetY = countY( this->m_teamAmout, fa->m_Actor->m_info.group, (fa->team_def-1)%3+1, fa->data );
					if ( fa->m_Actor->moveTo( targetX, targetY ) )
					{
						fa->m_Actor->m_info.btStations = fa->data;
						fa->m_Actor->setOriginPos( targetX, targetY );
                		fa->action_status = ACTION_STATUS_FINISH;
					}
				}
				break;
			case BATTLE_EFFECT_TYPE_PLAY_ANIMATION:// 对象身上播放指定动画
                {
					stringstream ss;
					ss << "sm_effect_" << fa->data << ".spr";
					NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath(ss.str().c_str())];
                    NDAnimationGroup* effect = [[NDAnimationGroup alloc] initWithSprFile:file];
					addSkillEffectToFighter(fa->m_Actor,effect,0,g_ArrayEfectProp[fa->data].iPos,g_ArrayEfectProp[fa->data].bRevers);
                    [effect release];
                }
                fa->action_status = ACTION_STATUS_FINISH;
				break;
			case BATTLE_EFFECT_TYPE_LIFE:
				fa->m_Actor->m_bHardAtk=false;
				fa->m_Actor->hurted(fa->data);
				NDLog(@"hurt %d",fa->data);
				fa->m_Actor->setCurrentHP((fa->m_Actor->m_info.nLife)+(fa->data));
				if (fa->m_Actor->m_info.nLife > 0) {// hurt
					fa->m_Actor->setHurtOK(true);
					if(fa->data<0){
...
void Battle::moveTeam(FightAction* action){
	...
	bool isOk=true;
	if(v_fighters){
		for(UInt32 i=0;i<v_fighters->size();i++){
	...
	if(isOk){
		action->action_status=ACTION_STATUS_FINISH;
		for (UInt32 i = 0; i < action->m_vCmdList.size(); i++)
		{
	...
	if (theActor->m_action == Fighter::MOVETOTARGET) {
		...
		if(action->m_Target){
			if (theActor->moveTo(action->m_Target->getOriginX() + coordw + roleOffset, action->m_Target->getOriginY())) {// 如果返回到达目的地
				//if(action->isDritical)
				//{
					//theActor->showAtkDritical();
				//}
...
//					}
					//处理技能光效// ljh test code
					//int effectId=skill->getSelfEffect();///1000;
                    //effectId = 21;//test code
                    ...
                    //    NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath(ss.str().c_str())];
					//	 NDAnimationGroup* effect = [[NDAnimationGroup alloc] initWithSprFile:file];
					//	 for (UInt32 i = 0; i < action->m_FighterList.size(); i++)
					//	 {	
		...
		switch(cmd->effect_type){
			case BATTLE_EFFECT_TYPE_LIFE:
				...
				if (fighter->m_info.nLife > 0) {// hurt
                    ...
                    //       nIndex %= 40;
                    //    }
					//	int iPos = g_ArrayEfectProp[nIndex].iPos;
					//	bool bRevers = g_ArrayEfectProp[nIndex].bRevers;
                    //	stringstream ss;
                    //	ss << "sm_effect_" << nIndex <<".spr";
                    //	NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath(ss.str().c_str())];
                    //	NDAnimationGroup* effect = [[NDAnimationGroup alloc] initWithSprFile:file];
                    //	
                    //	addSkillEffectToFighter(fighter,effect,0,iPos,bRevers);
                    //	[effect release];
					//}
				} else {// die
			...
			case BATTLE_EFFECT_TYPE_DODGE:
                ...
                {//播放闪避文字动画++Guosen 2012.6.28
                    NSString* file = [NSString stringWithUTF8String:NDPath::GetAniPath("sm_effect_25.spr")];
                    NDAnimationGroup* effect = [[NDAnimationGroup alloc] initWithSprFile:file];
			...
			case BATTLE_EFFECT_TYPE_CHANGE_POSTION://移位
				{
					...
					{
						fighter->m_info.btStations = cmd->data;
...
void Battle::addSubAniGroup(NDSprite* role, NDAnimationGroup* group, Fighter* f)
{
	...
	subAniGroup.frameRec = [[NDFrameRunRecord alloc] init];
...
void Battle::InitSpeedBar()
{
	...
	pic->Initialization(NDPath::GetImgPathBattleUI("menucatch.png"));
	...
	pic->Initialization(NDPath::GetImgPathBattleUI("menuescape.png"));
	...
	pic = new NDPicture(true);
	pic->Initialization(NDPath::GetImgPathBattleUI("menudefence.png"));
	...
	pic = new NDPicture(true);
	pic->Initialization(NDPath::GetImgPathBattleUI("menuattack.png"));
	...
	pic = new NDPicture(true);
	pic->Initialization(NDPath::GetImgPathBattleUI("menuwatch.png"));
