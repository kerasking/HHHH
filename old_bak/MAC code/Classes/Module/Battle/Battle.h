...
@interface ChatTextFieldDelegate : NSObject <UITextFieldDelegate>
{
	UITextField* tfChat;
}
...
public:
	...
	void draw() {
		...
		drawRectBar2(rect.origin.x, rect.origin.y, m_color, m_nNum, m_nNumMax, rect.size.width);		
	}
...
//		BS_CHOOSE_OUR_SIDE_MAG_ATK_EUDEMON,
	...
	bool IsPracticeBattle() { return m_battleType == BATTLE_TYPE_PRACTICE; }
	...
	void SetIsPlayBack(bool isPlayBack){this->isBattlePlayBack=isPlayBack;}
	
	bool isPlayBack(){return isBattlePlayBack;}
...
//	virtual void OnTableLayerCellFocused(NDUITableLayer* table, NDUINode* cell, unsigned int cellIndex, NDSection* section);
	...
	void setSceneCetner(int center_x,int center_y) {this->sceneCenterX=center_x;this->sceneCenterY=center_y;}
...
private:
	...
	Battle& operator = (const Battle& rhs) { return *this; }
	...
	BATTLE_ACTION m_defaultActionUser; Fighter* m_defaultTargetUser; int m_defaultSkillID;
	BATTLE_ACTION m_defaultActionEudemon; Fighter* m_defaultTargetEudemon; int m_defaultSkillIDEudemon;
...
public:
	void StartEraseInEffect() { if (eraseInOutEffect) eraseInOutEffect->StartEraseIn(); }
	void StartEraseOutEffect() { if (eraseInOutEffect) eraseInOutEffect->StartEraseOut(); }
	bool IsEraseInOutEffectComplete() { if (eraseInOutEffect) return eraseInOutEffect->isChangeComplete(); return true; }
	void CloseStatusDlg() {
		if (m_dlgStatus) {
	...
	void TurnStart();
	void TurnStartPet();
	int GetCurrentShowFighterId(){return currentShowFighter;}
	CoolDownRecord& GetSkillCoolDownRecord() { return m_recordCoolDown; }
private:
	// 战斗快捷栏
...
private:
	NDUILabel* m_lbText;
	ccColor4B m_clrFocus;			// 焦点背景色
	ccColor4B m_clrText;
	ccColor4B m_clrFocusText;		// 焦点文本色
};
