...
{
	return DownLoadServerListToDB(this->m_serverIP.c_str(), this->m_serverPort);
}
...
void NDBeforeGameMgr::RegiserCallBack( int errCode, std::string strtip  )/* errCode=(1,注册成功),(2,注册失败),(3,网络问题)*/
{
	...
	switch (errCode) 
	{
		...
		{ // 注册成功
            ...
            //this->SaveAccountPwdToDB(username.c_str(),password.c_str(),0);
            //ScriptMgrObj.excuteLuaFunc("RegisterSuccess", "Login_RegAccount");
    ...
    //ScriptMgrObj.excuteLuaFunc("RegisterFailed", "Login_RegAccount",strtip.c_str());
	SetUserName("");
	SetPassWord("");
...
///////////////////////////////////////////////////////////////////////////
bool 
NDBeforeGameMgr::LoginServer(const char* Login_Account,const char* Login_Server)
{
}
...
///////////////////////////////////////////////////////////////////////////
void NDBeforeGameMgr::FastRegisterAccount(void)
{
}
///////////////////////////////////////////////////////////////////////////
...
//				NDDirector::DefaultDirector()->ReplaceScene(scene);
				...
				SetUserName(username);
                this->SaveAccountPwdToDB(username.c_str(),username.c_str(),1);
...
//////////////////////////////////////////////////////////////////////
bool 
NDBeforeGameMgr::DownLoadServerListToDB(const char* pszIP, int nPort)
{
	NDSocket socServerList;
	// 套接字初始化,并连接
	if (!socServerList.Connect(pszIP, nPort, true) ) 
	{
		NDLog(@"连接服务器列表失败,ip:%s, port:%d", pszIP, nPort);
		return false;
	}
	NDTransData data(MB_SERVER_INFO_REQUEST);
	data << (unsigned short)(true);
	socServerList.Send(&data);
	int iMsgLen = 0;
	do 
	{
...
{
    //创建server数据表
    if(!CSqliteDBMgr::shareInstance().IsExistTable("server")){
        char *sql = "CREATE TABLE server(login_name text primary key,type integer,show_name text,login_ip text,port integer,login_time integer default 0, area_name text, state integer)";
	...
	{	
        ...
        //大区数据写入数据库
        char *sql = "REPLACE INTO server(login_name,type,show_name,login_ip,port,area_name,state) VALUES";
        char szValue[512] = "";
        sprintf(szValue,"(\'%s\',%d,\'%s\',\'%s\',%d,\'%s\',%d);",bigArea.c_str(),0,bigArea.c_str(),ips.c_str(),iPort,"",0);
		...
		for (int j = 0; j < available; j++) 
		{
            ...
            char *sqlserver = "REPLACE INTO server(login_name,type,show_name,login_ip,port,area_name,state) VALUES";
            sprintf(szValue,"(\'%s\',%d,\'%s\',\'%s\',%d,\'%s\',%d);",serverNameTrans.c_str(),1,serverNameDisplay.c_str(),ips.c_str(),iPort,bigArea.c_str(),cState);
            std::string strSqlServer = sqlserver;
            strSqlServer+=szValue;
            if(!CSqliteDBMgr::shareInstance().ExcuteSql(strSqlServer.c_str())){
                return;
            }
		}
...
////////////////////////////////////////////////////////////
void 
NDBeforeGameMgr::InitAccountTable()
{
    //创建server数据表
...
void
NDBeforeGameMgr::SaveAccountPwdToDB(const char* pszName, const char* pszPwd, int nType)
{
    if(!pszName){
        return;
    }
...
////////////////////////////////////////////////////////
//nType: 1表示快速注册的帐号，其他表示用户主动注册的帐号
/*void 
NDBeforeGameMgr::SaveAccountPwdToDB(const char* pszName, const char* pszPwd, int nType)
{
    if (!pszName) {
    ...
    char szValue[256] = "";
    char *sqlaccount = "REPLACE INTO account(name text primary key,type integer,pwd text,login_time integer) VALUES";
    sprintf(szValue,"(\'%s\',%d,\'%s\',%d);",pszName,nType,strPwd.c_str(),0);
    std::string strSqlAccount = sqlaccount;
    strSqlAccount+=szValue;
    if(!CSqliteDBMgr::shareInstance().ExcuteSql(strSqlAccount.c_str())){
        return;
...
bool NDBeforeGameMgr::CheckClientVersion( const char* szURL )
{
    ...
    //NSString *strIniPath1 = [NSString stringWithFormat:@"%s", NDPath::GetAppResFilePath("version.ini").c_str()];
    //NSLog(@"sssss:%@",strIniPath1);
...
{ 
	...
	string installVersionIniPath	=  NDPath::GetAppResFilePath("/") + NDPath::GetRootResDirName() + SZ_VERINI_PATH;
	...
	if ( !file )
	{
        ...
        //先判断文件是否是被损坏的，原来的目录已经存在，则先把原有的目录删除，再拷贝文件
        /*
        [[NSFileManager defaultManager] 
         copyItemAtPath:[NSString stringWithUTF8String:NDPath::GetAppResFilePath(NDPath::GetRootResDirName()).c_str()] 
         toPath:[NSString stringWithUTF8String:NDPath::GetResourceFilePath(NDPath::GetRootResDirName()).c_str()]  error:NULL ];
        */
        CopyRes();
	}
	else
