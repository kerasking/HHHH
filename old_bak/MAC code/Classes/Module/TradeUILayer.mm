...
//		std::vector<std::string> vec_str; vec_str.push_back("请输入银币");
//		view->SetEdit(1, vec_id, vec_str);
//		view->Show();
//	} else if (button == this->m_btnOurEMoney) { // 输入金币
//		NDUICustomView *view = new NDUICustomView;
//		view->Initialization();
//		view->SetTag(TAG_INPUT_YUANBAO);
//		view->SetDelegate(this);
//		std::vector<int> vec_id; vec_id.push_back(1);
//		std::vector<std::string> vec_str; vec_str.push_back("请输入金币");
//		view->SetEdit(1, vec_id, vec_str);
//		view->Show();
//	} else {
//		for (uint r = 0; r < ITEM_ROW; r++) {
...
public:
	...
	void Initialization() override
	{
		...
		for(int i = 0; i < 2; i++)
		{
			...
			vImgFile.push_back(NDPath::GetImgPathNew("btn_bg1.png"));
			vImgCustomRect.push_back(CGRectMake(0, 0, 38, 38));
			vOffsetPoint.push_back(CGPointMake(0, 0));
			vImgFile.push_back(NDPath::GetImgPathNew(i==0 ? "bag_bagemoney.png" : "bag_bagmoney.png"));
...
public:
	...
	void Initialization() override
	{
		...
		NDPicture *pic = pool.AddPicture(NDPath::GetImgPathNew("tradedlg_textback.png"), w, h);
		NDUIImage *img = new NDUIImage;
		img->Initialization();
		img->SetPicture(pic, true);
		...
		pic = pool.AddPicture(NDPath::GetImgPathNew("trade_amount.png"));
...
void NewTradeLayer::Initialization()
{
	...
	NDPicture *pic = pool.AddPicture(NDPath::GetImgPathNew("trade_money_bg.png"), 192, 20);
	...
	NDPicture *picMoneys = pool.AddPicture(NDPath::GetImgPathNew("trade_moneys.png"));
	...
	for (int i = 0; i < 2; i++) 
	{
		...
		img = new NDUIImage;
		img->Initialization();
		img->SetPicture(pool.AddPicture(NDPath::GetImgPathNew("trade_money_bg.png"), fw, fh), true);
...
void NewTradeScene::Initialization()
{
	...
	NDPicture *pic = pool.AddPicture(NDPath::GetImgPathNew("newui_btn.png"));
	
	NDPicture *picCommit = pool.AddPicture(NDPath::GetImgPathNew("trade_commit.png"));
