...
{
	m_SubAnimationGroups	= new CCMutableArray<NDAnimationGroups*>();
...
{
	if (frameRunRecord->runCount >= m_nEnduration - 1) 
	{
		frameRunRecord->runCount = 0;
		return true;
	}

	frameRunRecord->runCount++;
...
void NDFrame::drawHeadAt(CGPoint pos)
{
	...
	{
		NDFrameTile *frameTile = m_FrameTiles->objectAtIndex(i);
		NDTileTableRecord *record = animationGroup->getTileTable()->objectAtIndex(frameTile->getTableIndex());
...
void NDFrame::run(float scale)
{
	...
	NDAnimation *animation = m_BelongAnimation;
	NDAnimationGroup *animationGroup = animation->getBlongAnimationGroup();
