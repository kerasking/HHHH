...
{
	m_Frames = new CCMutableArray<NDFrame*>();
}
...
void NDAnimation::runWithRunFrameRecord(NDFrameRunRecord* runFrameRecord, bool needDraw, float drawScale)
{
	...
	{		
		...
		if (runFrameRecord->getNextFrameIndex() != 0 && runFrameRecord->getCurrentFrameIndex() == 0) 
		{
			...
			{
				NDFrame* frame = m_Frames->lastObject();
				if (needDraw) 
				{
					frame->run(drawScale);
				}
			...
			{
				NDFrame* frame = m_Frames->objectAtIndex(0);
				if (needDraw) 
				{
					frame->run(drawScale);
				}
		...
		//获取动画的当前帧
		NDFrame *frame = m_Frames->objectAtIndex(runFrameRecord->getCurrentFrameIndex());
		...
		if (frame->enableRunNextFrame(runFrameRecord)) 
		{
			...
			//取下一帧
			frame =  m_Frames->objectAtIndex(runFrameRecord->getNextFrameIndex());
...
void NDAnimation::SlowDown(unsigned int multi)
{
	if (m_Frames->count()) 
	{
		...
		{
			NDFrame *frame = m_Frames->objectAtIndex(i);
