...
, m_UnpassPoint(NULL)
{
	...
	m_Animations		= new CCArray<NDAnimation*>();
	m_TileTable			= new CCArray<NDTileTableRecord*>();
...
void NDAnimationGroup::setReverse(bool newReverse);
{
	...
	{
		NDAnimation *animation = m_Animations->objectAtIndex(i);
...
{
	NDAnimation* firstAni = m_Animations->objectAtIndex(0);
	NDFrame* firstFrame = firstAni->getFrames()->objectAtIndex(0);
