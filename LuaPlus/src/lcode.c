...
static void invertjump(FuncState *fs, expdesc *e)
{
	...
	lua_assert(
			testTMode(GET_OPCODE(*pc)) && GET_OPCODE(*pc) != OP_TESTSET && GET_OPCODE(*pc) != OP_TEST);
...
static int constfolding(OpCode op, expdesc *e1, expdesc *e2)
{
	...
	case OP_UNM:
		...
		break;
	case OP_BAND:
		r = (unsigned int) v1 & (unsigned int) v2;
		break;
	case OP_BOR:
		r = (unsigned int) v1 | (unsigned int) v2;
		break;
...
void luaK_posfix(FuncState *fs, BinOpr op, expdesc *e1, expdesc *e2)
{
	...
	case OPR_POW:
		...
		break;
	case OPR_BAND:
		codearith(fs, OP_BAND, e1, e2);
		break;
	case OPR_BOR:
		codearith(fs, OP_BOR, e1, e2);
		break;
	case OPR_BXOR:
		codearith(fs, OP_BXOR, e1, e2);
		break;
	case OPR_BSHL:
		codearith(fs, OP_BSHL, e1, e2);
		break;
	case OPR_BSHR:
		codearith(fs, OP_BSHR, e1, e2);
		break;
	case OPR_EQ:
