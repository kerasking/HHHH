...
void luaV_execute(lua_State *L, int nexeccalls)
{
	...
	for (;;)
	{
		...
		case OP_LEN:
		{
			...
			{ /* try metamethod */
				Protect(
						if (!call_binTM(L, rb, luaO_nilobject, ra, TM_LEN)) luaG_typeerror(L, rb, "get length of");)
			}
			}
			continue;
		}
		...
		case OP_EQ:
		{
			...
			TValue *rc = RKC(i);
			Protect(
					if (equalobj(L, rb, rc) == GETARG_A(i)) dojump(L, pc, GETARG_sBx(*pc));)
			pc++;
			continue;
		}
		case OP_LT:
		...
		case OP_TAILCALL:
		{
			...
			case PCRLUA:
			{
				...
				lua_assert(
						L->top == L->base + clvalue(func)->l.p->maxstacksize);
		...
		case OP_FORLOOP:
		{
			...
			if (luai_numlt(0, step) ?
					luai_numle(idx, limit) : luai_numle(limit, idx))
			{
				dojump(L, pc, GETARG_sBx(i));
				/* jump back */
				setnvalue(ra, idx);
