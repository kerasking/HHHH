...
public:
    ...
    //获取可视的行数和区域
    void GetVisibleLineRange(int& first/*开始行*/,int& last/*结束行*/,CRect& rect/*区域*/);
...
protected:
    ...
    //获取显示的宽度
    int GetLineWidth(int iStart/*文本开始位置*/,int iEnd/*文本结束位置*/);
...
private:
    ...
    void GetItemVisAtt(int iLineIndex/*行*/,int startIndex,int endIndex,CRect& retRect,CRect& selRect,string& retStr);
    ...
    bool m_bSpecialLink;//是否允许有特殊链接(默认false)
    //end
