...
/**
 ...
 @param error 错误码，如果error为0，则代表API执行成功，否则失败。错误码涵义请查看NdComPlatformError文件
 @param leaderboardInfoList  返回所有的排行 榜单列表
 */
- (void)getAllLeaderBoardsDidFinish:(int)error  result:(NdLeaderBoardInfoList*)leaderboardInfoList;
...
/**
 ...
 @param error 错误码，如果error为0，则代表API执行成功，否则失败。错误码涵义请查看NdComPlatformError文件
 @param achievementList 返回分页的成就列表信息
 */
- (void)getAllAchievementsDidFinish:(int)error result:(NdAchievementInfoList*)achievementList;
