...
const KData getMd5String(const KData & buf)
{
	...
	MD5Context _context;
	MD5Init( &_context );	
	MD5Update( &_context, reinterpret_cast < unsigned const char* >( buf.getData() ),
			  buf.length() );
	MD5Final( digest, &_context );
	return KData( reinterpret_cast < char* > (digest), 16 );	
}
...
void getMd5Digit(const unsigned char *buf, unsigned char *ret, int len)
{
	MD5Context _context;
	MD5Init( &_context );	
	MD5Update( &_context, buf, len );
	MD5Final( ret, &_context );
}
