...
function f.GetMapPortal(nMapId, nPortalIndex)
	...
	for i, v in ipairs(idlist) do
		local portal_index	= _G.ConvertN(_G.GetDataBaseDataN("portal", nMapId, DB_PORTAL.INDEX));
		if portal_index == nPortalIndex then
			nCellX	= _G.ConvertN(_G.GetDataBaseDataN("portal", nMapId, DB_PORTAL.PORTALX));
			nCellY	= _G.ConvertN(_G.GetDataBaseDataN("portal", nMapId, DB_PORTAL.PORTALY));
			break;
		end
	end
