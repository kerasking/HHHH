...
function p.onClickItem(uiNode, tag)
	for i = 1, #p.TagTrain do
		if tag == p.TagTrain[i] then
			MsgRoleTrain.sendTrain(p.mPetId, i);
			return true;
		end
	end
...
function p.refreshInfo()
	...
	if CheckP(levelV) then
		levelV:SetText(ConvertS(RolePet.GetPetInfoS(p.mPetId, PET_ATTR.PET_ATTR_NAME)));--SafeN2S(p.getPetLevel(p.mPetId)));
	end
...
function  p.getTrainMag(nPetId)
	return RolePet.GetPetInfoN(nPetId,PET_ATTR.PET_ATTR_MAGICL_FOSTER);
end
