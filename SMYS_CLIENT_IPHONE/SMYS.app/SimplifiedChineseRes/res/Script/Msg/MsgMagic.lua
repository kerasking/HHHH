...
function p.getRoleMatrixCount() 
	local count = p.getRoleMatrixInfoDataN(p.RoleMagicInfo.MATRIX_COUNT);
	if (count == nil) then
		count = 0;
	end
	return count;
...
function p.setRoleMatrixCount(count) 
	p.setRoleMatrixInfoDataN(p.RoleMagicInfo.MATRIX_COUNT, count);
end
...
function p.getRoleMatrixList()
	local count = p.getRoleMatrixCount();
	local lst = {};
	for i = 1, count do
		local m = {};
		--m.index = i;
...
function p.setRoleMatrixUpdate(id, level)
	...
	local index = -1;
	for i = 1, count do
		local lid = p.getRoleMatrixListDataN(i, p.RoleMatrixList.ID);
		if (lid == id) then
			index = i;
		end
	end
	...
	if (index > 0) then
		p.setRoleMatrixListDataN(index, p.RoleMatrixList.LEVEL, level);
	end
...
function p.setRoleMatixStation(m)
	...
	--[[
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.ID,  m.id);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S1, m[1]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S2, m[2]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S3, m[3]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S4, m[4]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S5, m[5]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S6, m[6]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S7, m[7]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S8, m[8]);
	p.setRoleMatrixStationDataN( p.RoleMatrixStation.S9, m[9]);
	]]--
...
function p.getRoleAttackCount() 
	local count = p.getRoleAttackInfoDataN(p.RoleMagicInfo.ATTACK_COUNT);
...
function p.getRoleAttackList()
	local count = p.getRoleAttackCount();
	local lst = {};
	for i = 1, count do
		local m = {};
		--m.index = i;
...
function p.setRoleAttackCount(count) 
	p.setRoleAttackInfoDataN(p.RoleMagicInfo.ATTACK_COUNT, count);
end
...
function p.setRoleAttackList(listTable, count)
	...
	p.setRoleMAttackCount(count);
end

-- upgrade
function p.setRoleAttackUpdate(id, level)
	local count = p.getRoleAttackCount();
	LogInfo("setRoleAttackUpdate id:%d level:%d", id, level)
	local index = -1;
	for i = 1, count do
		local lid = p.getRoleAttackListDataN(i, p.RoleAttackList.ID);
		if (lid == id) then
			index = i;
			LogInfo("setRoleAttackUpdate index:%d", index)
		end
	end
	
	if (index > 0) then
		p.setRoleAttackListDataN(index, p.RoleAttackList.LEVEL, level);
	end
