...
- (void)NextFrame:(int)nTotalFrames
{
	if (_bSetPlayRange)
	{
		_currentFrameIndex	= _nStartFrame < _nEndFrame ? _currentFrameIndex+1 : _currentFrameIndex-1;
		_nextFrameIndex		= _nStartFrame < _nEndFrame ? _currentFrameIndex+1 : _currentFrameIndex-1;
		...
		{
			_currentFrameIndex	= nTotalFrames;
		}
...
{
	if (frameRunRecord.runCount >= _enduration - 1) 
	{
