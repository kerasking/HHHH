...
@synthesize name = _name, layerCount = _layerCount, columns = _columns, rows = _rows, aniGroupParams = _aniGroupParams,unitSize=_unitSize,
mapTiles = _mapTiles, obstacles = _obstacles, sceneTiles = _sceneTiles,bgTiles=_bgTiles, switchs = _switchs, animationGroups = _animationGroups;
	...
	for ( int lay = 0; lay < _layerCount; lay++) 
		for (uint r = 0; r < _rows; r++) 
			...
			{					
				...
				CustomCCTexture2D *tile = [[CustomCCTexture2D alloc] init];
				tile.texture	= [[MapTexturePool defaultPool] addImage:imageName keepData:true];	//[[CCTextureCache sharedTextureCache] addImage:imageName keepData:true]; 
				...
				tile.horizontalReverse	= reverse;				
