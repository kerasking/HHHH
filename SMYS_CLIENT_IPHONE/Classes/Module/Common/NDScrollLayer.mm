...
void NDUILabelScrollLayer::SetText(const char* text, unsigned int leftInterval/*=0*/, unsigned int rightInterval/*=0*/, unsigned int height/*=0*/, ccColor4B fontColor/* = ccc4(58, 58, 58, 255)*/)
{
	if (!text) return;
...
bool NDUIContainerScrollLayer::refresh(float distance)
{
	...
	if (m_bVisibleScroll && m_imageScroll && this->m_fMaxChange != 0.0f) 
	{
		...
		if (rect.size.height > 0.0f)
		{
			...
			NDPicture *pic = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("new_scroll.png"), 0, rect.size.height);
...
void NDUIContainerScrollLayer::refreshContainer(float defaultChangeRange/*=0.0f*/)
{
	...
	if (m_bVisibleScroll && fMax > 0.0f) 
	{
		...
		NDPicture *pic = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("new_scroll.png"), 0, fHeight);
...
bool NDUIContainerScrollLayer::SetContent(const char *text, ccColor4B color/*=ccc4(0, 0, 0, 255)*/, unsigned int fontsize/*=12*/)
{
