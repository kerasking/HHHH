...
void DirectKey::Initialization()
{
	...
	m_picNormal = pool.AddPicture(GetImgPathNew("nav_normal.png"));
	
	m_picNormal->SetColor(ccc4(125, 125, 125, 125));
	
	m_picDown = pool.AddPicture(GetImgPathNew("nav_down.png"));
	...
	vImgFiles[1] = GetImgPathNew("nav_suo.png");
...
void DirectKey::OnButtonClick(NDUIButton* button)
{
	...
	if (button == m_btnShrink && pNode && pNode->IsKindOfClass(RUNTIME_CLASS(NDUILayer))) 
	{
		...
		CGPoint beginTouch = ((NDUILayer*)pNode)->m_beginTouch;
...
bool DirectKey::CheckCell(int iCellX, int iCellY)
{
	...
	return [mapdata canPassByRow:iCellY andColumn:iCellX];
}
