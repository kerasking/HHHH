...
void SynInfoUILayer::Initialization()
{
	...
	switch (camp) {
		case CAMP_TYPE_TU:
			imgCamp->SetPicture(pool.AddPicture(GetImgPathNew("tu.png")), true);
			break;
		case CAMP_TYPE_TANG:
			imgCamp->SetPicture(pool.AddPicture(GetImgPathNew("tang.png")), true);
			break;
		case CAMP_TYPE_SUI:
			imgCamp->SetPicture(pool.AddPicture(GetImgPathNew("sui.png")), true);
			break;
		default:
			break;
	}
	...
	imgSynBroadcast->SetPicture(pool.AddPicture(GetImgPathNew("syn_broadcast.png")), true);
	...
	m_btnQuitSyn->SetImage(pool.AddPicture(GetImgPathNew("btn_quit_syn.png")), false, CGRectZero, true);
	...
	if (player.getSynRank() > SYNRANK_MEMBER) {
		...
		m_btnResign->SetImage(pool.AddPicture(GetImgPathNew("resign.png")), false, CGRectZero, true);
		this->AddChild(m_btnResign);
		m_btnResign->SetBackgroundPicture(pool.AddPicture(GetImgPathNew("btn_bg1.png")), NULL, true, CGRectMake(0, 3, 35, 35), true);
	}
	...
	if (player.getSynRank() >= SYNRANK_VICE_LEADER) {
		...
		m_btnChgSynBoard->SetImage(pool.AddPicture(GetImgPathNew("btn_modify_note.png")), false, CGRectZero, true);
