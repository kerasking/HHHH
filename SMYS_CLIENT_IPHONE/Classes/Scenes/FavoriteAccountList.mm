...
void AccountListRecord::SetFrameRect(CGRect rect)
{
	...
	if (rect.size.width != 0 && rect.size.width != 0 && !m_picSel)
		m_picSel = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("bg_focus.png"), rect.size.width*2/3-20, rect.size.height-4);
}
...
void NewFavoriteAccountList::Initialization()
{
	...
	for (int i = 0; i < 2; i++) 
	{
		NDPicture *pic = picpool.AddPicture(GetImgPathNew("create&login_text.png"));
	...
	imgTableBg->SetPicture(picpool.AddPicture(GetImgPathNew("server_bg.png"), 420, 220), true);
