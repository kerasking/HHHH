...
bool CUIPetTip::Init(int nWidth)
{
	...
	NDPicture* pPicBg = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("bag_bag_bg.png"), nWidth, 279);
	if (!pPicBg) {
		return false;
	}
...
bool CUIPet::SetTabNodePic(TabNode* pTabNode, int nStartX)
{
	...
	NDPicture *pPic = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("newui_text.png"));
	NDPicture *pPicFocus = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew("newui_text.png"));
	if (!pPic || !pPicFocus) {
		return false;
	}
