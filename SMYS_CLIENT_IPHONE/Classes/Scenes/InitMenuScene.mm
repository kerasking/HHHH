...
#define BGIMAGE [NSString stringWithFormat:@"%s", GetImgPath("login_bk.png")]
#define ARROWIMAGELEFT [NSString stringWithFormat:@"%s", GetImgPath("arrow_left.png")]
#define ARROWIMAGERIGHT [NSString stringWithFormat:@"%s", GetImgPath("arrow_right.png")]
...
InitMenuScene::InitMenuScene()
{
	...
	*/
	...
	m_leftArrowImage = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew(file_img_left_normal));
	
	m_rightArrowImage = m_leftArrowImage->Copy();
	
	m_leftArrowHightlightImage = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew(file_img_left_hightlight));
...
{	
	...
	m_Layer->SetBackgroundImage(GetImgPathNew(file_img_bg));
	this->AddChild(m_Layer);
	
	NDPicturePool& picPool = *(NDPicturePool::DefaultPool());
	
	NDPicture *picBtnImg = picPool.AddPicture(GetImgPathNew(file_img_btn_bg));
	...
	*/
	...
	m_focusImg[ctBegin] = picPool.AddPicture(GetImgPathNew(file_img_menu_focus));
	...
	NDPicture *freePic = picPool.AddPicture(GetImgPathNew(file_img_copyright));
	...
	NDPicture *copyrightPic = picPool.AddPicture(GetImgPathNew(file_img_copyright));
...
NDPicture* InitMenuScene::GetMenuTextPic(std::string text, bool focus, bool first, bool bParticular/*=false*/)
{
	...
	*/if (text == NDCommonCString("fast")) 
	...
	res = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew(file_img_menu_text));
...
NDPicture* InitMenuScene::GetVerTextPic(char cChar)
{
	...
	res = NDPicturePool::DefaultPool()->AddPicture(GetImgPathNew(file_img_ver));
