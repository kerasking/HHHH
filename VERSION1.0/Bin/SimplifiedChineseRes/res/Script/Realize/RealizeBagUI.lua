...
function p.LoadUI()
	...
	local petNameContainer = p.getContainerById(p.TagListName)
	if CheckP(petNameContainer) then
		...
		petNameContainer:SetLuaDelegate(p.OnUIEvent);
	end
...
function p.LoadPageView()
	local svc	= p.getContainerById(p.TagListPage);
...
function p.OnUIEventClickPetName(uiNode, uiEventType, param)
	if uiEventType == NUIEventType.TE_TOUCH_BTN_CLICK then
		...
		if CheckP(view) then
			...
			local containter	= p.getContainerById(p.TagListName);
			if CheckP(containter) then
				containter:ScrollViewById(nPetId);
			end
			
			containter = p.getContainerById(p.TagLeftContainer);
			if CheckP(containter) then
				containter:ScrollViewById(nPetId);
			end
		end
...
function p.OnUIEvent(uiNode, uiEventType, param)
	...
	elseif uiEventType == NUIEventType.TE_TOUCH_SC_VIEW_IN_BEGIN then
		...
		elseif tag == p.TagListName then
			container = p.getContainerById(p.TagLeftContainer);
			container:ScrollViewById(nPetId) 
		elseif tag == p.TagLeftContainer then
			container = p.getContainerById(p.TagListName);
			container:ScrollViewById(nPetId) 
		end 
	end
	return true;
...
function p.OnUIEventItemInfo(uiNode, uiEventType, param)
	...
	if uiEventType == NUIEventType.TE_TOUCH_BTN_CLICK then
		...
		--穿装备
			...
			MsgItem.SendPackDaoFa(nItemId, nPetId);
			ShowLoadBar();
...
function p.ContainerAddPetName(nPetId)
	...
	local container	= p.getContainerById(p.TagListName);
	if not CheckP(container) then
		return;
	end
...
function p.ChangeItemInfo(nItemId, bEquip)
	...
	--装备
		...
		if not bEquip then
			if nPetId > 0 then
				local equipBtn = CreateButton("btn_normal.png", "btn_select.png", GetTxtPri("Common_equip"),
			...
			local drogBtn = CreateButton("btn_normal.png", "btn_select.png", GetTxtPri("Common_drop"),
									CGRectMake(nStartX, nHeight, nBtnW, nBtnH), 12);
			...
			if nPetId > 0 then
				local unequipBtn = CreateButton("btn_normal.png", "btn_select.png", GetTxtPri("GemXieXia"),
										CGRectMake(nStartX, nHeight, nBtnW, nBtnH), 12);
		...
		local sendBtn = CreateButton("btn_normal.png", "btn_select.png", GetTxtPri("Common_PetShow"),
								CGRectMake(nStartX, nHeight, nBtnW, nBtnH), 12);
			...
			--if (not (dragerSrc.srcType == p.DrageType.Equip and dragerDst.srcType ==  p.DrageType.Equip) ) then
				...
				if (srcId and dstId and dstId > 0 and srcId > 0) then
					...
					if (dragerDst.srcType == p.DrageType.Equip and dragerSrc.srcType == p.DrageType.BagItem and srcId and srcId > 0 and p.mCurrentPetId) then
						MsgItem.SendPackDaoFa(p.mCurrentPetId, srcId);
					elseif (dragerDst.srcType == p.DrageType.BagItem and dragerSrc.srcType == p.DrageType.Equip and srcId and srcId > 0 and p.mCurrentPetId) then
						MsgItem.SendUnDaoFa(srcId, p.mCurrentPetId);
					end
				end
