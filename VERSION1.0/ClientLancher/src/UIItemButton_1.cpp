...
void CUIItemButton::ChangeItem(unsigned int unItemId)
{
	...
	unsigned int nItemType = 0;//GetItemInfoN(unItemId, ITEM_TYPE);
...
void CUIItemButton::ChangeItemType(unsigned int unItemType)
{
	...
	unsigned int nIconIndex = 0;//GetItemDBN(unItemType, DB_ITEMTYPE_ICONINDEX);
	if (nIconIndex > 0)
	{
		NDPicture* pic = 0;//ItemImage::GetSMItem(nIconIndex);
		if (pic)
		{
			...
			if (!m_bShowAdapt)
			{
				...
				CGRect rect = CGRectMake((frame.size.width - size.width) / 2,
						(frame.size.height - size.height) / 2, size.width,
						size.height);
...
void CUIItemButton::RefreshItemCount()
{
	...
	unsigned int nItemType = 0;//GetItemInfoN(m_unItemId, ITEM_TYPE);

	if (true)//IsItemCanChaiFen(nItemType))
	{
...
void CUIItemButton::ChangeItemCount(unsigned int unItemCount)
{
	...
	if (!pNode || !pNode->IsKindOfClass(RUNTIME_CLASS(NDUILabel)))
	{
		...
		pLabel->SetFrameRect(
				CGRectMake(0.125 * rect.size.width, 0.5 * rect.size.height,
						0.75 * rect.size.width, 0.333 * rect.size.height));
		...
		if (parentNode)
		{
			if (parentNode - IsKindOfClass(RUNTIME_CLASS(NDUILayer)))
			{
				...
				if (m_image || m_combinepicImg)
				{
					...
					//if (!IsTabSel() && ((m_touchDownStatus != TouchDownImage) || !m_touched || NULL == m_touchDownImage) )
					{
						if (m_useCustomRect)
						{
							...
							{
								float fScale =
										NDDirector::DefaultDirector()->GetScaleFactor();
				...
				if (uiLayer->GetFocus() == this /*|| IsTabSel()*/)
				{
					...
					else if (m_focusStatus
							== FocusRimImage /*&& m_bFocusEnable*/)
					{
						...
						int d = 0;
						if (scrRect.origin.x
								< NDDirector::DefaultDirector()->GetWinSize().width
										/ 2)
						{
							d = 1;
						}
					...
					else if (m_focusStatus == FocusImage
...
void CUIItemButton::SetItemFrameRect(CGRect rect)
{
	SetFrameRect(rect);
}
