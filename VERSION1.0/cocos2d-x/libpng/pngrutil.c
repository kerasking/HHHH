...
void Decrypt( png_byte * buf );//++Guosen
...
png_handle_IHDR(png_structp png_ptr, png_infop info_ptr, png_uint_32 length)
{
   ...
   png_read_data(png_ptr, buf, length);
	if ( buf[10] != 0 )//++Guosen 2012.8.21 //判定是加密的PNG文件
	{
		Decrypt( &buf[2] );
		buf[10] = 0;
		nWidth = buf[0] * 0x1000000 + buf[1] * 0x10000 + buf[2] * 0x100 + buf[3];
		nWidth -= 0x400;//宽度减去1024
		buf[0] = ( nWidth / 0x1000000 ) & 0xff;
		buf[1] = ( nWidth / 0x10000 ) & 0xff;
		buf[2] = ( nWidth / 0x100 ) & 0xff;
		buf[3] = ( nWidth / 0x1 ) & 0xff;
   	}
   png_calculate_crc(png_ptr, buf, length);//++CRC验证
...
//++Guosen 2012.8.21//解密独特的解密文件
void Decrypt( png_byte * buf )
{
	...
	nCode[1] = 0x64656E61;
	nCode[2] = 0;
	szEncrypt = (png_byte * )nCode;

	for ( ; i < 8; ++i )
	{
		png_byte cXor = szEncrypt[i];
		png_byte cOpt = buf[i];
