...
function p.getPicture(nId)
	...
	local iconIndex = GetDataBaseDataN("pet_config", nPetType,DB_PET_CONFIG.ICON);
	
	local rtn = p.GetPicByIconIndex(iconIndex);
...
function p.getEnablePic()
	...
	return pool:AddPicture(GetSMImgPath("bg/bg_front_nor.png"), false)
...
LogInfo("PicBy:%d" ,iconIndex)
  local pool = DefaultPicPool();
  if CheckP(pool) then
    local pic = pool:AddPicture(GetSMImgPath("mix/Figure_Portrait.png"), false);
	if CheckP(pic) then
	  pic:Cut(CGRectMake(
