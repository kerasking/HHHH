 ...
 * EOWNERDEAD.
 * Note ¨C Your application must always check the return code from pthread_mutex_lock() for
 * a mutex initialized with the PTHREAD_MUTEX_ROBUST attribute.
 * ¡ö The new owner of this mutex should make the state protected by the mutex consistent.
 * This state might have been left inconsistent when the previous owner terminated.
 * ¡ö If the new owner is able to make the state consistent, call
 ...
 * ¡ö If the new owner is not able to make the state consistent, do not call
 ...
 * ¡ö If the thread that acquires the lock with EOWNERDEAD terminates without unlocking the
