...
static int matchbracketclass(lua_WChar c, const lua_WChar *p,
		const lua_WChar *ec)
{
	int sig = 1;
	if (*(p + 1) == '^')
...
static const lua_WChar *match(MatchState *ms, const lua_WChar *s,
		const lua_WChar *p);
		...
		const lua_WChar *s2, size_t l2)
{
	...
	else
	{
		...
		while (l1 > 0
				&& (init = (const lua_WChar *) lp_wmemchr(s1, *s2, l1)) != NULL)
		{
...
static const lua_WChar* lua_WChar_pbrk(const lua_WChar* str,
		const lua_WChar* control)
{
	while (*str != 0)
	{
...
static int str_find_aux(lua_State *L, int find)
{
	...
	lua_WChar_pbrk(p, (const lua_WChar*) SPECIALS) == NULL))
...
static lua_WChar wLUA_INTFRMLEN[] =
{'l', 'l', 0};
...
while (strfrmt < strfrmt_end)
{
	...
	{ /* format item */
		lua_WChar form[MAX_FORMAT] =
		{ 0 }; /* to store the format (`%...') */
		lua_WChar buff[MAX_ITEM] =
		{ 0 }; /* to store the formatted item */
		...
		case 'x':
		{
			...
			{ /* \xxx */
				...
				do
				{
					...
					++i;
					ch = towlower(str[i]);
				} while (i - start < numDigits
						&& (iswdigit((wint_t) ch) || (ch >= 'a' && ch <= 'f')));
				luaL_addwchar(&b, c);
				--i;
				//				  needUnicodeZero = 0;
			}
