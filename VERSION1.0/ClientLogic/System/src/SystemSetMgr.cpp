...
bool SystemSetMgr::initSystemSetTable()
{
	...
	{
		char *sql =
				"CREATE TABLE system_set(name text primary key,value1 integer,value2 text)";
...
bool SystemSetMgr::Set(const char *key, int value)
{
	...
	{
		char* pszSql = "REPLACE INTO system_set(name,value1,value2) VALUES";

		char szValue[256] = "";

		std::string strString = "";

		sprintf(szValue, "(\'%s\',%d,\'%s\');", key, value, strString.c_str());
...
bool SystemSetMgr::Set(const char *key, const char *value)
{
	if (initSystemSetTable())
	{
		...
		sprintf(szValue, "(\'%s\',%d,\'%s\');", key, 0, value);
...
int SystemSetMgr::GetNumber(const char* szKey, int nDefaultValue/*=0*/)
{
	if (initSystemSetTable())
	{
		...
		strSql += szKey;
		strSql += "\';";
		int nRowNum = CSqliteDBMgr::shareInstance().SelectData(strSql.c_str(),
				1);
		if (nRowNum < 1)
		{
			NDLog("系统设置获取失败!%s", szKey);
...
const char *SystemSetMgr::GetString(const char *key,
