...
void PetSkillIconLayer::Initialization(PetSkillInfoLayer* skillInfoLayer)
{
	...
	imgBg->SetPicture(pool.AddPicture(GetImgPathNew("bag_bag_bg.png")), true);
	...
	for (int i = 0 ; i < ROW_COUNT; i++) {
		...
		for (int j = 0; j < COL_COUNT; j++) {
			...
			if (it != vec_id.end()) {
				...
				btn->SetTag(-1);
				btn->SetImage(pool.AddPicture(GetImgPathNew("lockstate.png")), true, CGRectMake(1, 1, 40, 40), true);
			}
