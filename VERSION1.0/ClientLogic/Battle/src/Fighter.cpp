...
//	int team=this->m_info.btBattleTeam-1;
	this->originX=countX(teamAmout, this->m_info.group, m_info.btBattleTeam, this->m_info.btStations);
	this->originY=countY(teamAmout, this->m_info.group, m_info.btBattleTeam, this->m_info.btStations);	
...
void Fighter::updatePos()
{
	...
	if (!isVisibleStatus) {
		...
		if (lbHover) {
			CGSize sizeStr = getStringSize(lbHover->GetText().c_str(), DEFAULT_FONT_SIZE);
	...
	if (this->lb_FighterName) {
		CGSize sizeStr = getStringSize(lb_FighterName->GetText().c_str(), DEFAULT_FONT_SIZE);
	...
	if (lb_skillName){
		...
		CGSize sizeStr =getStringSize(lb_skillName->GetText().c_str(), DEFAULT_FONT_SIZE);
		lb_skillName->SetFrameRect(CGRectMake(pt.x +(m_role->GetWidth()/2), pt.y - (m_role->GetHeight()/2) - sizeStr.height, sizeStr.width, sizeStr.height));
	}
}
...
void Fighter::showFighterName(bool b)
{
	if (b) {
		...
		if(role){
			...
			CGSize sizefighterName = getStringSize(GetRole()->m_name.c_str(), DEFAULT_FONT_SIZE);
			//fighterName->SetTag(TAG_FIGHTER_NAME);
			lb_FighterName->SetFrameRect(CGRectMake(pt.x - sizefighterName.width / 2, pt.y - m_role->GetHeight() - sizefighterName.height, sizefighterName.width, sizefighterName.height));
...
void Fighter::showSkillName(bool b)
{
	if (b) {
		...
		CGSize sizeSkillName = getStringSize(m_strSkillName.c_str(), DEFAULT_FONT_SIZE);
		//lb_skillName->SetTag(TAG_SKILL_NAME);
		lb_skillName->SetFontSize(DEFAULT_FONT_SIZE);
		lb_skillName->SetFrameRect(CGRectMake(pt.x +(m_role->GetWidth()/2), pt.y - (m_role->GetHeight()/2) - sizeSkillName.height, sizeSkillName.width, sizeSkillName.height));
...
void Fighter::showHoverMsg(const char* str)
{
	...
	if (!lbHover) {
		...
		lbHover->SetFrameRect(CGRectMake(pt.x - sizeStr.width / 2, pt.y - m_role->GetHeight(), sizeStr.width, sizeStr.height));
