...
void DramaChatLayer::SetFigure(std::string filename, bool bReverse)
{
	...
	{
		NDPicture* pic = NDPicturePool::DefaultPool()->AddPicture(filename.c_str());
...
void DramaChatLayer::SetContent(std::string content, int nFontSize, int nFontColor)
{
	NDNode* node = this->GetChild(m_nTagContent);
