...
bool DramaScene::AddSprite(int nKey, std::string filename)
{
	...
	{
		NDAsssert(0);
		ScriptMgrObj.DebugOutPut("DramaScene::AddSprite m_mapSprite.end() != it filename[%s]key[%d]", filename.c_str(), nKey);
		return false;
	}
	...
	{
		NDAsssert(0);
		ScriptMgrObj.DebugOutPut("DramaScene::AddSprite filename.empty key[%d]", nKey);
		return false;
	}
	...
	{
		delete sprite;
		ScriptMgrObj.DebugOutPut("DramaScene::AddSprite !AddNodeToMap filename[%s]key[%d]", filename.c_str(), nKey);
		return false;
	}
...
bool DramaScene::PushScene(int nKey, DramaTransitionScene* scene)
{
	...
	{
		NDAsssert(0);
		ScriptMgrObj.DebugOutPut("DramaScene::PushScene m_mapScene.end() != it key[%d]", nKey);
		return false;
	}
...
void DramaScene::SetChatTitle(bool bLeft, std::string title, int nFontSize, int nFontColor)
{
	DramaChatLayer*& chat = bLeft ? m_chatlayerLeft : m_chatlayerRight;
	if (!chat)
	{
...
void DramaScene::SetChatTitleBySpriteKey(bool bLeft, int nKey, int nFontSize, int nFontColor)
{
	std::string title = "";
...
void DramaScene::SetChatContent(bool bLeft, std::string content, int nFontSize, int nFontColor)
{
	DramaChatLayer*& chat = bLeft ? m_chatlayerLeft : m_chatlayerRight;
	if (!chat)
	{
...
{
	bool bSucces = false;
	for (MAP_SPRITE_IT it = m_mapSprite.begin(); 
		 it != m_mapSprite.end(); 
		 it++) 
	{
		if (node == it->second)
...
{
	bool bSucces = false;
	for (MAP_MONSTER_IT it = m_mapMonster.begin(); 
		 it != m_mapMonster.end(); 
		 it++) 
	{
		if (node == it->second)
...
{ 
	...
	if (uinode == m_chatlayerLeft ||
		uinode == m_chatlayerRight ||
		uinode == m_dlgConfirm)
	{
		ProductClick();
		return true;
