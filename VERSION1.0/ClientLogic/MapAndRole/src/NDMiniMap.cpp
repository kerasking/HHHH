...
//	m_miniTaskPic->Initialization(GetImgPath("mini_quest.png"));
	//m_miniTaskPic->Cut(CCRectMake(0.0f, 0.0f, 37.0f, 18.0f));
...
void Radar::mapLogin()
{
	CCRect rectScreen = this->GetScreenRect();
	this->startX = rectScreen.origin.x;
	this->startY = rectScreen.origin.y;
	
	CCSize size = this->scene->GetSize();
	if (size.width > 480) {
		this->MINI_W = 480 / SCALE_X;
	} else {
	...
	CCPoint pos = this->m_role.GetPosition();
...
void Radar::drawRole()
{
	...
	if (this->timeCount % 12 < 6) {
		if (this->x >= 0 && this->x <= MINI_W && this->y >= 0 && this->y <= MINI_H) {
			DrawRecttangle(CCRectMake(this->x + this->startX - DARK_WH, this->y + this->startY - DARK_WH, LIGHT_WH, LIGHT_WH),
						   ccc4(252, 252, 2, 255));
		}
	} else {
		if (x >= 0 && x <= MINI_W && y >= 0 && y <= MINI_H) {
			DrawRecttangle(CCRectMake(x + startX - HALF_DARK_WH, y + startY - HALF_DARK_WH, DARK_WH, DARK_WH),
						   ccc4(251, 174, 40, 255));
		}
	}
	this->timeCount++;
...
void Radar::drawMiniQuest(int npcState, int x, int y) {
	if ((npcState & NPC_STATE_GRAY_EXCALMATORY_MARK) > 0) { // 绿色
		this->m_miniTaskPic->Cut(CCRectMake(4, 0, 4, 9));
		this->m_miniTaskPic->DrawInRect(CCRectMake(x, y, 4, 9));
	} else if ((npcState & NPC_STATE_COLOR_EXCALMATORY_MARK) > 0) { // 红色
		this->m_miniTaskPic->Cut(CCRectMake(0, 0, 4, 9));
		this->m_miniTaskPic->DrawInRect(CCRectMake(x, y, 4, 9));
	} else if ((npcState & NPC_STATE_COLOR_QUESTION_MARK) > 0) { // 问号
		this->m_miniTaskPic->Cut(CCRectMake(8, 0, 6, 9));
		this->m_miniTaskPic->DrawInRect(CCRectMake(x - 1, y - 1, 6, 9));
	}
}
...
//			DrawRecttangle(CCRectMake(tempX + startX - HALF_DARK_WH, tempY + startY - HALF_DARK_WH, DARK_WH, DARK_WH), ccc4(0, 255, 0, 255));
...
//			if (tempX >= 0 && tempX <= this->MINI_W && tempY >= 0 && tempY <= this->MINI_H) {
//				DrawRecttangle(CCRectMake(tempX + startX - HALF_DARK_WH, tempY + startY - HALF_DARK_WH, DARK_WH, DARK_WH), ccc4(255, 0, 0, 255));
//			}
//		}
//	}
}
...
//				DrawRecttangle(CCRectMake(tx, ty, DARK_WH, DARK_WH), clr);
//				rolePos.insert(key);
//			}
//		}
//	}
...
//		if (tempX >= 0 && tempX <= this->MINI_W && tempY >= 0 && tempY <= this->MINI_H) {
//			DrawRecttangle(CCRectMake(tempX + this->startX - 1, tempY + this->startY - 1, 3, 3),
//						   ccc4(33, 213, 236, 255));
//		}
//	}
}
...
// 	
// 	//CCRect miniMapRect = CCRectMake(80.0f, startY, MINI_W + DARK_WH, MINI_H + DARK_WH);
// 	
// 	s_instance = this;
// 	
// 	m_status = SS_SHOW;
...
// 	imgRadar->SetFrameRect(CCRectMake(0, 0, 172, 66));
// 	this->AddChild(imgRadar);
// 	
// 	m_btnMap = new NDUIButton;
// 	m_btnMap->Initialization();
// 	m_btnMap->SetFrameRect(CCRectMake(0.0f, 0.0f, 106.0f, 51.0f));
// 	m_btnMap->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathBattleUI("show_map.png"), false), true, CCRectMake(4, 4, 30, 30), true);
// 	m_btnMap->SetDelegate(this);
// 	this->AddChild(m_btnMap);
// 	
// 	NDUIRecttangle* rectBg = new NDUIRecttangle();
// 	rectBg->Initialization();
// 	rectBg->SetFrameRect(CCRectMake(110.0f, 6.0f, MINI_W + DARK_WH, MINI_H + DARK_WH));
// 	rectBg->SetColor(ccc4(125, 125, 125, 125));
// 	this->AddChild(rectBg);
// 	
// 	NDUIPolygon* border = new NDUIPolygon();
// 	border->Initialization();
// 	border->SetFrameRect(CCRectMake(110.0f, 6.0f, MINI_W + DARK_WH, MINI_H + DARK_WH));
...
// 	m_lbMapName->SetFrameRect(CCRectMake(36.0f, 15.0f, 480.0f, 17.0f));
...
// 	m_lbServerName->SetFrameRect(CCRectMake(36.0f, 31.0f, 80.0f, 17.0f));
// 	this->AddChild(m_lbServerName);
// 	
// 	m_radar = new Radar;
// 	m_radar->Initialization();
// 	m_radar->SetFrameRect(CCRectMake(111.0f, 7.0f, MINI_W, MINI_H));
// 	this->AddChild(m_radar);
// 	
// 	NDUIRecttangle* rectCoordBg = new NDUIRecttangle();
// 	rectCoordBg->Initialization();
// 	rectCoordBg->SetFrameRect(CCRectMake(111.0f, 42.0f, MINI_W + 1.0f, 16.0f));
...
// 	m_lbCoord->SetFrameRect(CCRectMake(117.0f, 42.0f, 50.0f, 17.0f));
// 	this->AddChild(m_lbCoord);
// 	
// 	m_btnShrink = new NDUIButton;
// 	m_btnShrink->Initialization();
// 	m_btnShrink->SetImage(NDPicturePool::DefaultPool()->AddPicture(GetImgPathBattleUI("shrink_map.png"), false), false, CCRectMake(0, 0, 0, 0), true);
// 	m_btnShrink->SetFrameRect(CCRectMake(86.0f, 46.0f, 41.0f, 41.0f));
...
// }
// 
// const CCPoint PT_BTN_SHRINK_SHOW = CCPointMake(394.0f, 46.0f);
// const CCPoint PT_BTN_SHRINK_HIDE = CCPointMake(442.0f, -6.0f);
// 
...
// 		CCRect rtFrame = this->GetFrameRect();
// 		CCRect rtBtnShrink = m_btnShrink->GetScreenRect();
// 		bool bOk = true;
// 		// x轴
