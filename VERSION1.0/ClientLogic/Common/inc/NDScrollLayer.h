...
public:
	...
	void SetText(const char* text, unsigned int leftInterval=0, unsigned int rightInterval=0, unsigned int height=0, ccColor4B fontColor = ccc4(58, 58, 58, 255));
...
public:
	...
	// 不支持动态加入或删除部分节点(若要这么做，必须保证原先的节点复位)
	...
	bool SetContent(const char *text, ccColor4B color=ccc4(0, 0, 0, 255), unsigned int fontsize=12);
private:
