...
void ChatManager::AddChatInfoRecord(std::string speaker,std::string text,int content_id,CHAT_CHANNEL_TYPE type)
{
...
void ChatManager::AddAllRecord()
{
	...
	for (; iter != m_records.end(); iter++)
	{
		...
		if(record)
		{
			if(currentChannel==CHAT_CHANNEL_ALL||record->GetChatType()==currentChannel)
			{
