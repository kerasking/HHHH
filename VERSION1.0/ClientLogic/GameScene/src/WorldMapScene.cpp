...
void WorldMapLayer::Initialization(int nMapId)
{
	...
	{
		PlaceNode *node = (PlaceNode *)(PlaceNode*)(PlaceNode*)m_mapData->getPlaceNodes()->objectAtIndex(i);
		...
		if (m_mapFilename.end() == m_mapFilename.find(node->getPlaceId()))
		{
			continue;
		}
		
		const char* path	= m_mapFilename[node->getPlaceId()].c_str();
		if (NULL == path)
		{
			continue;
		}
		
		cocos2d::CCTexture2D* tex	= cocos2d::CCTextureCache::sharedTextureCache()->addImage(path);
		if (NULL == tex)
		{
			continue;
		}
...
{
	PlaceNode* node = (PlaceNode*)m_mapData->getPlaceNodes()->objectAtIndex(nIndex);
	if (node)
	{
...
{	
	...
	CGPoint posCenter = ccp(winSize.width / 2 - pos.x, winSize.height / 2 + pos.y - m_mapData->getMapSize().height);
...
bool WorldMapLayer::TouchBegin(NDTouch* touch)
{
	...
	{
		PlaceNode* node = (PlaceNode*)m_mapData->getPlaceNodes()->objectAtIndex(i);
		CGRect btnRect = CGRectMake(node->getX(), 
									node->getY(), 
...
CGPoint WorldMapLayer::GetPlaceIdScreenPos(int placeId)
{
	...
	{		
		float fScaleFactor	= NDDirector::DefaultDirector()->GetScaleFactor();
		CGPoint pos = CGPointMake(node->getX() + node->getTexture()->getContentSizeInPixels().width / 2, 
								  node->getY() - node->getTexture()->getContentSizeInPixels().height);
