...
void ItemMgr::processItemInfo(NDTransData* data, int len)
{
	...
	(*data) >> itemCount; // 接收的物品个数

	for (int j = 0; j < itemCount; j++)
	{
		int itemID = 0;
		(*data) >> itemID; // 物品的Id 4个字节
		int ownerID = 0;
		(*data) >> ownerID; // 物品的所有者id 4个字节
		int itemType = 0;
		(*data) >> itemType; // 物品类型 id 4个字节
		int dwAmount = 0;
		(*data) >> dwAmount; // 物品数量/耐久度 4个字节
		int itemPosition = 0;
		(*data) >> itemPosition; // 物品位置 4个字节
		int btAddition = 0;
		(*data) >> btAddition; // 装备追加 4个字节
...
void ItemMgr::processItemAttrib(NDTransData* data, int len)
{
	...
	int action = 0;
	(*data) >> action; // 行为,目前只有_ITEM _AMOUNT
