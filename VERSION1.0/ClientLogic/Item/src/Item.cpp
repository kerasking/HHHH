...
{
	m_nID = 0;					// 物品的Id
	m_nOwnerID = 0;				// 物品的所有�??�id
	m_nItemType = 0;				// 物品类型 id
	m_nAmount = 0;				// 物品数量/耐久�??
	m_nPosition = 0;				// 物品位置
	m_nAddition = 0;				// 装备追加
	m_nBindState = 0;			// 绑定状�????
	m_nHole = 0;				// 装备有几个洞
	m_nCreateTime = 0;			// 创建时间
	m_nAge = 0;					// 骑宠寿命
	m_bIsActive = false;
}
...
{
	stringstream sb;
	int type = getIdRule(m_nItemType, Item::ITEM_TYPE); // 物品类型
		...
		int whichStore)
{
	...
	{	// 装备
		...
		tempInt1 = getdwAmountShow(item1.getAmount_limit());
		tempInt2 = getdwAmountShow(item2.getAmount_limit());	// �??大�??�久�??
