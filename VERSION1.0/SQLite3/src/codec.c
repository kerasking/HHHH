...
void CodecRC4(Codec* codec, unsigned char* key, int keylen,
		unsigned char* textin, int textlen, unsigned char* textout)
{
	int i;
	int j;
	int t;
...
void CodecGetMD5Binary(Codec* codec, unsigned char* data, int length,
...
#if CODEC_TYPE == CODEC_TYPE_AES256
void CodecGetSHABinary(Codec* codec, unsigned char* data, int length,
		unsigned char* digest)
{
...
void CodecAES(Codec* codec, int page, int encrypt,
		unsigned char encryptionKey[KEYLENGTH], unsigned char* datain,
		int datalen, unsigned char* dataout)
{
	...
	int j;
	int direction =
			(encrypt) ? RIJNDAEL_Direction_Encrypt : RIJNDAEL_Direction_Decrypt;
	int len = 0;
...
#if CODEC_TYPE == CODEC_TYPE_AES256
	RijndaelInit(codec->m_aes, RIJNDAEL_Direction_Mode_CBC, direction, pagekey,
			RIJNDAEL_Direction_KeyLength_Key32Bytes, initial);
#else
...
void CodecPadPassword(Codec* codec, char* password, int pswdlen,
...
{
	CodecGenerateEncryptionKey(codec, userPassword, passwordLength,
			codec->m_readKey);
}
...
void CodecGenerateEncryptionKey(Codec* codec, char* userPassword,
		int passwordLength, unsigned char encryptionKey[KEYLENGTH])
{
#if CODEC_TYPE == CODEC_TYPE_AES256
...
void CodecEncrypt(Codec* codec, int page, unsigned char* data, int len,
		int useWriteKey)
{
