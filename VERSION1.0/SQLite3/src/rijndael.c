...
{ 99, 124, 119, 123, 242, 107, 111, 197, 48, 1, 103, 43, 254, 215, 171, 118,
		...
		192, 183, 253, 147, 38, 54, 63, 247, 204, 52, 165, 229, 241, 113, 216,
		49, 21, 4, 199, 35, 195, 24, 150, 5, 154, 7, 18, 128, 226, 235, 39, 178,
		117, 9, 131, 44, 26, 27, 110, 90, 160, 82, 59, 214, 179, 41, 227, 47,
		132, 83, 209, 0, 237, 32, 252, 177, 91, 106, 203, 190, 57, 74, 76, 88,
		207, 208, 239, 170, 251, 67, 77, 51, 133, 69, 249, 2, 127, 80, 60, 159,
		168, 81, 163, 64, 143, 146, 157, 56, 245, 188, 182, 218, 33, 16, 255,
		243, 210, 205, 12, 19, 236, 95, 151, 68, 23, 196, 167, 126, 61, 100, 93,
...
{  };
		...
		UINT8* initVector)
{
	...
	/* Check the mode */
	if ((mode != RIJNDAEL_Direction_Mode_CBC)
			&& (mode != RIJNDAEL_Direction_Mode_ECB)
			&& (mode != RIJNDAEL_Direction_Mode_CFB1))
		return RIJNDAEL_UNSUPPORTED_MODE;
	rijndael->m_mode = mode;

	/* And the direction */
	if ((dir != RIJNDAEL_Direction_Encrypt)
			&& (dir != RIJNDAEL_Direction_Decrypt))
	...
	if (rijndael->m_direction == RIJNDAEL_Direction_Decrypt)
		RijndaelKeyEncToDec(rijndael);
		...
		UINT8 *outBuffer)
{
	...
	if (rijndael->m_state != RIJNDAEL_State_Valid)
	...
	case RIJNDAEL_Direction_Mode_CBC:
		((UINT32*) block)[0] = ((UINT32*) rijndael->m_initVector)[0]
				^ ((UINT32*) input)[0];
		((UINT32*) block)[1] = ((UINT32*) rijndael->m_initVector)[1]
				^ ((UINT32*) input)[1];
		((UINT32*) block)[2] = ((UINT32*) rijndael->m_initVector)[2]
				^ ((UINT32*) input)[2];
		((UINT32*) block)[3] = ((UINT32*) rijndael->m_initVector)[3]
				^ ((UINT32*) input)[3];
		RijndaelEncrypt(rijndael, block, outBuffer);
		input += 16;
		for (i = numBlocks - 1; i > 0; i--)
		{
			((UINT32*) block)[0] = ((UINT32*) outBuffer)[0]
					^ ((UINT32*) input)[0];
			((UINT32*) block)[1] = ((UINT32*) outBuffer)[1]
					^ ((UINT32*) input)[1];
			((UINT32*) block)[2] = ((UINT32*) outBuffer)[2]
					^ ((UINT32*) input)[2];
			((UINT32*) block)[3] = ((UINT32*) outBuffer)[3]
					^ ((UINT32*) input)[3];
			outBuffer += 16;
			RijndaelEncrypt(rijndael, block, outBuffer);
			input += 16;
		...
		UINT8 *outBuffer)
{
	...
	if (rijndael->m_state != RIJNDAEL_State_Valid)
		return RIJNDAEL_NOT_INITIALIZED;
	if ((rijndael->m_mode != RIJNDAEL_Direction_Mode_CFB1)
			&& (rijndael->m_direction == RIJNDAEL_Direction_Encrypt))
		return RIJNDAEL_BAD_DIRECTION;
...
void RijndaelKeySched(Rijndael* rijndael, UINT8 key[_MAX_KEY_COLUMNS][4])
{
	...
	for (j = 0; (j < uKeyColumns) && (r <= rijndael->m_uRounds);)
	{
		...
		{
			*((UINT32*) rijndael->m_expandedKey[r][t]) =
					*((UINT32*) tempKey[j]);
		}
...
void RijndaelKeyEncToDec(Rijndael* rijndael)
{
	...
	for (r = 1; r < rijndael->m_uRounds; r++)
	{
		...
		*((UINT32*) w) = *((UINT32*) U1[w[0]]) ^ *((UINT32*) U2[w[1]])
				^ *((UINT32*) U3[w[2]]) ^ *((UINT32*) U4[w[3]]);
...
void RijndaelEncrypt(Rijndael* rijndael, UINT8 a[16], UINT8 b[16])
{
	...
	*((UINT32*) temp[0]) = *((UINT32*) (a))
			^ *((UINT32*) rijndael->m_expandedKey[0][0]);
	*((UINT32*) temp[1]) = *((UINT32*) (a + 4))
			^ *((UINT32*) rijndael->m_expandedKey[0][1]);
	*((UINT32*) temp[2]) = *((UINT32*) (a + 8))
			^ *((UINT32*) rijndael->m_expandedKey[0][2]);
	*((UINT32*) temp[3]) = *((UINT32*) (a + 12))
			^ *((UINT32*) rijndael->m_expandedKey[0][3]);
	*((UINT32*) (b)) = *((UINT32*) T1[temp[0][0]]) ^ *((UINT32*) T2[temp[1][1]])
			^ *((UINT32*) T3[temp[2][2]]) ^ *((UINT32*) T4[temp[3][3]]);
	*((UINT32*) (b + 4)) = *((UINT32*) T1[temp[1][0]])
			^ *((UINT32*) T2[temp[2][1]]) ^ *((UINT32*) T3[temp[3][2]])
			^ *((UINT32*) T4[temp[0][3]]);
	*((UINT32*) (b + 8)) = *((UINT32*) T1[temp[2][0]])
			^ *((UINT32*) T2[temp[3][1]]) ^ *((UINT32*) T3[temp[0][2]])
			^ *((UINT32*) T4[temp[1][3]]);
	*((UINT32*) (b + 12)) = *((UINT32*) T1[temp[3][0]])
			^ *((UINT32*) T2[temp[0][1]]) ^ *((UINT32*) T3[temp[1][2]])
			^ *((UINT32*) T4[temp[2][3]]);
	for (r = 1; r < rijndael->m_uRounds - 1; r++)
	{
		*((UINT32*) temp[0]) = *((UINT32*) (b))
				^ *((UINT32*) rijndael->m_expandedKey[r][0]);
		*((UINT32*) temp[1]) = *((UINT32*) (b + 4))
				^ *((UINT32*) rijndael->m_expandedKey[r][1]);
		*((UINT32*) temp[2]) = *((UINT32*) (b + 8))
				^ *((UINT32*) rijndael->m_expandedKey[r][2]);
		*((UINT32*) temp[3]) = *((UINT32*) (b + 12))
				^ *((UINT32*) rijndael->m_expandedKey[r][3]);

		*((UINT32*) (b)) = *((UINT32*) T1[temp[0][0]])
				^ *((UINT32*) T2[temp[1][1]]) ^ *((UINT32*) T3[temp[2][2]])
				^ *((UINT32*) T4[temp[3][3]]);
		*((UINT32*) (b + 4)) = *((UINT32*) T1[temp[1][0]])
				^ *((UINT32*) T2[temp[2][1]]) ^ *((UINT32*) T3[temp[3][2]])
				^ *((UINT32*) T4[temp[0][3]]);
	...
	*((UINT32*) temp[0]) = *((UINT32*) (b))
			^ *((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds - 1][0]);
	*((UINT32*) temp[1]) = *((UINT32*) (b + 4))
			^ *((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds - 1][1]);
	*((UINT32*) temp[2]) = *((UINT32*) (b + 8))
			^ *((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds - 1][2]);
	*((UINT32*) temp[3]) = *((UINT32*) (b + 12))
			^ *((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds - 1][3]);
	b[0] = T1[temp[0][0]][1];
	b[1] = T1[temp[1][1]][1];
	b[2] = T1[temp[2][2]][1];
	b[3] = T1[temp[3][3]][1];
	...
	b[15] = T1[temp[2][3]][1];
	*((UINT32*) (b)) ^=
			*((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds][0]);
	*((UINT32*) (b + 4)) ^=
			*((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds][1]);
	*((UINT32*) (b + 8)) ^=
			*((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds][2]);
	*((UINT32*) (b + 12)) ^=
			*((UINT32*) rijndael->m_expandedKey[rijndael->m_uRounds][3]);
}
...
void RijndaelDecrypt(Rijndael* rijndael, UINT8 a[16], UINT8 b[16])
{
	...
	*((UINT32*) (b)) = *((UINT32*) T5[temp[0][0]]) ^ *((UINT32*) T6[temp[3][1]])
			^ *((UINT32*) T7[temp[2][2]]) ^ *((UINT32*) T8[temp[1][3]]);
	*((UINT32*) (b + 4)) = *((UINT32*) T5[temp[1][0]])
			^ *((UINT32*) T6[temp[0][1]]) ^ *((UINT32*) T7[temp[3][2]])
			^ *((UINT32*) T8[temp[2][3]]);
	*((UINT32*) (b + 8)) = *((UINT32*) T5[temp[2][0]])
			^ *((UINT32*) T6[temp[1][1]]) ^ *((UINT32*) T7[temp[0][2]])
			^ *((UINT32*) T8[temp[3][3]]);
	*((UINT32*) (b + 12)) = *((UINT32*) T5[temp[3][0]])
			^ *((UINT32*) T6[temp[2][1]]) ^ *((UINT32*) T7[temp[1][2]])
			^ *((UINT32*) T8[temp[0][3]]);
	for (r = rijndael->m_uRounds - 1; r > 1; r--)
	{
		...
		*((UINT32*) (b)) = *((UINT32*) T5[temp[0][0]])
				^ *((UINT32*) T6[temp[3][1]]) ^ *((UINT32*) T7[temp[2][2]])
				^ *((UINT32*) T8[temp[1][3]]);
		*((UINT32*) (b + 4)) = *((UINT32*) T5[temp[1][0]])
				^ *((UINT32*) T6[temp[0][1]]) ^ *((UINT32*) T7[temp[3][2]])
				^ *((UINT32*) T8[temp[2][3]]);
	...
	*((UINT32*) temp[0]) = *((UINT32*) (b))
			^ *((UINT32*) rijndael->m_expandedKey[1][0]);
	*((UINT32*) temp[1]) = *((UINT32*) (b + 4))
			^ *((UINT32*) rijndael->m_expandedKey[1][1]);
	*((UINT32*) temp[2]) = *((UINT32*) (b + 8))
			^ *((UINT32*) rijndael->m_expandedKey[1][2]);
	*((UINT32*) temp[3]) = *((UINT32*) (b + 12))
			^ *((UINT32*) rijndael->m_expandedKey[1][3]);
	b[0] = S5[temp[0][0]];
	b[1] = S5[temp[3][1]];
	b[2] = S5[temp[2][2]];
	b[3] = S5[temp[1][3]];
