...
// ET-EXPORT
	...
	struct ETClassBegin##classname { \
		ETClassBegin##classname() {	ScriptMgrObj.AddRegClassFunc(reg); }	\
		static bool reg()	\
		{					\
	...
	return true;}} obj##classname; \

	//----------------------------------------------------------------------------
	#define ETSTRUCTBEGIN(structname) \
	struct ETStructBegin##structname { \
		ETStructBegin##structname() { ScriptMgrObj.AddRegClassFunc(reg); } \
		static bool reg() \
		{ \
