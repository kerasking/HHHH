...
//===========================================================================
	void NDDataTransThread::Start(const char* ipAddr, int port)
	{
		...
		{
			cocos2d::CCLog( "the thread is paused, can't start, you can call resume methord!" );
		}
		else
		{
			#if BLOCK_SOCKET == 1
			...
			{
				cocos2d::CCLog( "connect server failed, maybe the socket is connected!" );
				return;
			}
		}
	}
		...
		while (m_operate != ThreadStatusStoped)
		{
			...
			else
			{
				...
				else
				{
					...
					cocos2d::CCLog( "ready for read msg, msgid[%d], msglen[%d]", code, len );
					...
					do
					{
						...
						if (hasRead != len-6)
						{
							cocos2d::CCLog( "read msg error, msgid[%d], msglen[%d]", code, len );
							continue;
						}
