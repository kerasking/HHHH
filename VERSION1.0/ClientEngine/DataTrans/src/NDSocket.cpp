...
bool NDSocket::Connect(const char* address, unsigned short port, bool blocking)
{
	...
	int keepAlive = 1;
	::setsockopt(m_iSocket, SOL_SOCKET, SO_KEEPALIVE, (char*) &keepAlive,
			sizeof(keepAlive));

	int nSndBuf = 256 * 1024;
	if (::setsockopt(m_iSocket, SOL_SOCKET, SO_SNDBUF, (char*) &nSndBuf,
			sizeof(nSndBuf)))
	{
		CloseSocket (m_iSocket);
		return false;
	...
	if (::setsockopt(m_iSocket, SOL_SOCKET, SO_RCVBUF, (char*) &nRcvBuf,
			sizeof(nRcvBuf)))
	{
		CloseSocket (m_iSocket);
		return false;
	...
	addr_in.sin_port = htons(m_sPort);
	if (::connect(m_iSocket, (sockaddr*) &addr_in, sizeof(addr_in)) != 0)
	{
		...
		if (select(m_iSocket + 1, NULL, &fdsWrite, NULL, &_timeval) <= 0)
		{
			...
			::getsockopt(m_iSocket, SOL_SOCKET, SO_ERROR, (char*) &error,
					(int*) &size);
			if (error != 0)
			{
				cocos2d::CCLog("Error: connect to %s:%d failed!\n",
						m_szIPAddress.c_str(), m_sPort);
				return false;
			}
		}
	}
...
//===========================================================================
bool NDSocket::Receive(unsigned char* data, int& iReadedLen,
		int iReadLen/*=512*/)
{
	if (!IsConnected())
	{
...
bool NDSocket::Send(NDTransData* data)
{
	...
	ret = ::send(m_iSocket, (const char *) data->GetBuffer(), data->GetSize(),
			0);
	if (ret == SOCKET_ERROR)
	{
		cocos2d::CCLog("fail to connect server!");
...
bool NDSocket::SetSocketNotBlock(unsigned int sock)
{
	...
	::setsockopt(m_iSocket, SOL_SOCKET, SO_SNDTIMEO,
			(const char*) &iConnTimeOut, sizeof(int));
	::setsockopt(m_iSocket, SOL_SOCKET, SO_RCVTIMEO,
			(const char*) &iConnTimeOut, sizeof(int));
