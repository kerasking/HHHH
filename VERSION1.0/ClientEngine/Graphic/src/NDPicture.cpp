...
// 			}
	...
	void NDPicture::SetCoorinates()
	{
		if (m_texture) 
		{
			if (m_reverse) 
			{
				m_coordinates[0] = (m_cutRect.origin.x + m_cutRect.size.width) / m_texture->getPixelsWide();
	...
	void NDPicture::Cut(CGRect rect)
	{
		if (m_texture) 
		{
			...
			if (rect.origin.x + rect.size.width <= m_texture->getContentSizeInPixels().width && 
				rect.origin.y + rect.size.height <= m_texture->getContentSizeInPixels().height) 
			{
				bCutSucess = true;
				m_cutRect = rect;
			}
