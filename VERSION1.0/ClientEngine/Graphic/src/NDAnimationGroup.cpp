...
: m_nType(0)
, m_nIdentifer(0)
//, m_bReverse(0) ///< 临时性注释 郭浩
//, m_Animations(NULL) ///< 临时性注释 郭浩
, m_Images(NULL)
//, m_TileTable(NULL) ///< 临时性注释 郭浩
, m_RuningSprite(NULL)
, m_UnpassPoint(NULL)
{
	m_Position			= CGPointMake(0, 0);
	m_RunningMapSize	= CGSizeMake(0, 0);
//	m_Animations		= CCArray::array(); ///< 临时性注释 郭浩
	//m_TileTable			= CCArray::array(); ///< 临时性注释 郭浩
	m_Images			= new vector<std::string>();

//	m_Animations->retain(); ///< 临时性注释 郭浩
	//m_TileTable->retain(); ///< 临时性注释 郭浩
}

NDAnimationGroup::~NDAnimationGroup()
{
//	CC_SAFE_RELEASE(m_Animations); ///< 临时性注释 郭浩
//	CC_SAFE_RELEASE(m_TileTable); ///< 临时性注释 郭浩
...
//		m_TileTable->addObject(record); ///< 临时性注释 郭浩
		record->release();	
	}
}
...
//		m_Animations->addObject(animation); ///< 临时性注释 郭浩
		animation->release();
	}
