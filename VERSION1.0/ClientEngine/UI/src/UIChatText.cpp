		...
		const char* text)
{
	...
	if (!channel_str.empty())
	{
		textNodeList.push_back(
				ChatNode(false,
						CreateLabel("[", CHAT_FONTSIZE, ccc4(255, 0, 0, 255),
								0)));
		textNodeList.push_back(
				ChatNode(false,
						CreateLabel(channel_str.c_str(), CHAT_FONTSIZE,
								ccc4(255, 0, 0, 255), 0)));
		textNodeList.push_back(
				ChatNode(false,
						CreateLabel("]", CHAT_FONTSIZE, ccc4(255, 0, 0, 255),
	...
	if (CHAT_CHANNEL_TYPE(channel) != CHAT_CHANNEL_SYS)
	{
		textNodeList.push_back(
				ChatNode(false,
						CreateLabel(speaker, CHAT_FONTSIZE,
								ccc4(255, 0, 0, 255), speakerID)));
		textNodeList.push_back(
				ChatNode(false,
						CreateLabel(":", CHAT_FONTSIZE, ccc4(255, 0, 0, 255),
								speakerID)));
	...
	while (*text != '\0')
	{
		...
		if (retAnalysis)
		{
			if (type == ChatItem)
			{
				textNodeList.push_back(
						ChatNode(brk,
								CreateLabel("]", CHAT_FONTSIZE, clr, m_idItem),
								true));
			}
			type = ChatNone;
		...
		if (type == ChatItem)
		{
			textNodeList.push_back(
					ChatNode(brk,
							CreateLabel(word, CHAT_FONTSIZE, clr, m_idItem),
							true));
		}
		else
		{
			textNodeList.push_back(
					ChatNode(brk, CreateLabel(word, CHAT_FONTSIZE, clr, 0)));
		}
...
NDPicture* CUIChatText::CreateFacePicture(unsigned int index)
{
	...
	if (index < 25)
	{
		int row = index / 5;
		int col = index % 5;
		result = NDPicturePool::DefaultPool()->AddPicture(
				NDPath::GetImgPath("face.png"));
...
NDUIImage* CUIChatText::CreateFaceImage(const char* strIndex)
{
	...
	if (strlen(strIndex) >= 2)
	{
		...
		if (intIndex >= 0 && intIndex < 25)
		{
			...
			result->SetFrameRect(
					CGRectMake(0, 0, pic->GetSize().width,
							pic->GetSize().height));
		}
	}
	return result;
...
NDUILabel* CUIChatText::CreateLabel(const char* text, unsigned int fontSize,
...
bool CUIChatText::AnalysisRuleHead(const char*& text, ChatTextType &type,
		ccColor4B &textColor)
{
	bool result = false;
	if (*text == '<')
