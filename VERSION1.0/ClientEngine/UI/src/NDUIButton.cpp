...
//		this->AddChild(m_title);
	...
	{
		/*
		m_rimImageLT = NDPicturePool::DefaultPool()->AddPicture(GetImgPath("rim.png"));
		m_rimImageLT->SetReverse(true);
		m_rimImageLT->Rotation(PictureRotation270);
		
		m_rimImageRT = NDPicturePool::DefaultPool()->AddPicture(GetImgPath("rim.png"));
		m_rimImageRT->Rotation(PictureRotation90);
		
		m_rimImageLB= NDPicturePool::DefaultPool()->AddPicture(GetImgPath("rim.png"));
		m_rimImageLB->Rotation(PictureRotation270);
		
		m_rimImageRB = NDPicturePool::DefaultPool()->AddPicture(GetImgPath("rim.png"));
			...
			if (parentNode) 
			{
				if (parentNode-IsKindOfClass(RUNTIME_CLASS(NDUILayer))) 
				{
					...
					if (uiLayer->GetFocus() == this) 
					{
						...
						else if (m_focusStatus == FocusRimImage)
						{
					...
					{/*
						...
						if (!m_spriteArrow) 
						{
							...
							m_spriteArrow->Initialization(GetAniPath("button.spr"));
					...
					if (m_longTouched)
					{
						...
						if (pic)
						{
							pic->SetColor(ccc4(255, 255, 255, 125));
							float fScale = NDDirector::DefaultDirector()->GetScaleFactor();
