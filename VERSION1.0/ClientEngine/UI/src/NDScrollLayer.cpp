...
bool NDScrollLayer::OnLayerMove(NDUILayer* uiLayer, UILayerMove move, float distance)
{
	...
	if (!bVertical && !bHorizontal) return false;
...
{
	if (NDUILayer::DispatchTouchEndEvent(beginTouch, endTouch)) return true;
	
	NDScrollLayerDelegate *delegate = dynamic_cast<NDScrollLayerDelegate *> (this->GetDelegate());
...
void NDScrollLayer::SetHMoveSpeed()
{
	...
	for (unsigned int i = 0; i < size - 1; i++) 
	{
		...
		float dispass		= m_HMoves[secondindex].pos.x - m_HMoves[firstindex].pos.x;
		clock_t clockpass	= ClockTimeMinus(m_HMoves[secondindex].time, m_HMoves[firstindex].time);
		if (clockpass != 0)
		{
...
void NDScrollLayer::SetVMoveSpeed()
{
	...
	for (unsigned int i = 0; i < size - 1; i++) 
	{
		...
		double dispass		= m_VMoves[secondindex].pos.y - m_VMoves[firstindex].pos.y;
		clock_t clockpass	= ClockTimeMinus(m_VMoves[secondindex].time, m_VMoves[firstindex].time);
		if (clockpass != 0)
		{
			fSum += dispass / clockpass * 1000000.0f;
			uiTotal ++;
...
float NDScrollLayer::GetVMoveDistance()
{
	...
	double s = this->m_fVv0 * clockpass + fAccer * (clockpass * clockpass) / 2.0f;
...
bool NDUILabelScrollLayer::refresh(float distance)
{
	if (m_lbText ) 
	{
		...
		if (y > 0 || y+m_uiTextHeight < height) return false;
...
void NDUILabelScrollLayer::SetText(const char* text, unsigned int leftInterval/*=0*/, unsigned int rightInterval/*=0*/, unsigned int height/*=0*/, cocos2d::ccColor4B fontColor/* = ccc4(58, 58, 58, 255)*/)
{
	if (!text) return;
	
	SAFE_DELETE_NODE(m_lbText);
	
	m_uiTextHeight = NDUITextBuilder::DefaultBuilder()->StringHeightAfterFilter(text, this->GetFrameRect().size.width-leftInterval-rightInterval, 13);
	
	m_lbText = NDUITextBuilder::DefaultBuilder()->Build(text, 
														13, 
														CGSizeMake(this->GetFrameRect().size.width-leftInterval-rightInterval, m_uiTextHeight), 
														fontColor,
														false);
	
	
	
	m_lbText->SetFrameRect(CGRectMake(leftInterval, 0, 
									  this->GetFrameRect().size.width-leftInterval-rightInterval, 
									  m_uiTextHeight));
...
bool NDUIContainerScrollLayer::refresh(float distance)
{
	...
	for_vec(m_childrenList, std::vector<NDNode*>::iterator)
	{
	...
	if (m_bVisibleScroll && m_imageScroll && this->m_fMaxChange != 0.0f) 
	{
		...
		{
			rect.origin.y = tmpChange / this->m_fMaxChange * (this->GetFrameRect().size.height-rect.size.height);
			m_imageScroll->SetFrameRect(rect);
			
			NDPicture *pic = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPathNew("new_scroll.png"), 0, rect.size.height);
...
void NDUIContainerScrollLayer::refreshContainer(float defaultChangeRange/*=0.0f*/)
{
	float fMax = 0.0f;
	...
	if (m_bVisibleScroll && fMax > 0.0f) 
	{
		...
		float fHeight = fMax < selfHeight ?  0.0f : selfHeight / fMax * selfHeight;
		
		NDPicture *pic = NDPicturePool::DefaultPool()->AddPicture(NDPath::GetImgPathNew("new_scroll.png"), 0, fHeight);
...
bool NDUIContainerScrollLayer::SetContent(const char *text, cocos2d::ccColor4B color/*=ccc4(0, 0, 0, 255)*/, unsigned int fontsize/*=12*/)
{
