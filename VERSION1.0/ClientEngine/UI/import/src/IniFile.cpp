...
{
	size_t size = 0;
	FILE* pkFileHandle = NULL;
	pkFileHandle = fopen(m_strPath.c_str(), "rb");
	if (pkFileHandle == NULL)
	{
		return false;
	}
	fseek(pkFileHandle, 0, SEEK_END);
	size = ftell(pkFileHandle);
	fseek(pkFileHandle, 0, SEEK_SET);

	memset(strBuf, 0, iBufSize);
	size_t nReadNum = fread(strBuf, 1, iBufSize, pkFileHandle);
	fclose(pkFileHandle);
	if (nReadNum != size)
	{
		return false;
...
void CIniFile::WriteFile()
{
	...
	for (int keynum = 0; keynum < m_sections.size(); keynum++)
	{
		if (!m_sections[keynum].vv.empty())
		{
			...
			for (int valuenum = 0; valuenum < m_sections[keynum].vv.size();
					valuenum++)
			{
...
int CIniFile::GetLineAmount(const char* keyname)
{
	...
	if (iter != m_section2Index.end() && iter->second < m_sections.size())
	{
...
bool CIniFile::SetValueLine(const char* keyname, const char* valueline,
		bool create)
{
	Section2Index::iterator iter = m_section2Index.find(keyname);
	if (iter != m_section2Index.end() && iter->second < m_sections.size())
	...
	else
	{
		...
		m_section2Index.insert(
				Section2Index::value_type(keyname, m_sections.size() - 1));

	}
	return true;
}

CIniFile::ValueVector::iterator CIniFile::FindValueVector(
		CIniFile::ValueVector& vv, const char* valuename)
{
...
bool CIniFile::SetValue(const char* keyname, const char* valuename,
...
bool CIniFile::SetValueI(const char* keyname, const char* valuename, int value,
		bool create)
{
	//CString temp;
	//temp.Format("%d",value);
...
bool CIniFile::SetValueF(const char* keyname, const char* valuename,
