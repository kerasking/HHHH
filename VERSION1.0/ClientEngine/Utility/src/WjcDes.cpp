...
bool Encrypt(const uint_8* data,uint_32 nSrcLen,uint_8* out,uint_32& nDstLen,const uint_8* key)	{
	if ((NULL==data) || (NULL==out) || (0==nSrcLen) || (0==nDstLen) || (NULL==key))	{
		return false;
	}
	
	uint_32 n = nSrcLen>>3;
	uint_32 nLeft = nSrcLen&0x7;
	uint_32 nIndex = 0;
	uint_32 nOut = 0;
	uint_8 i;
	DesPriv_SetKey(DES_KEY);
	for (i=0;(i<n)&&((nOut+8)<nDstLen);++i)	{
		//des(data+nIndex,key,out+nOut,DES_ENCRYPT);
		Des_Run((char*)out+nOut,(char*)data+nIndex,ENCRYPT);
		nIndex += 8;
		nOut += 8;
	}
	if ((0 != nLeft)&&((nOut+8)<nDstLen))	{
		uint_8 buf[8] = {0};
		memcpy(buf,data+nIndex,nLeft);
		//des(data+nIndex,key,out+nOut,DES_ENCRYPT);
		Des_Run((char*)out+nOut,(char*)buf,ENCRYPT);
		nOut += 8;
	}
...
char* EncryptPwd(char* pPwd,char*key)	{
	...
	char* pEncode = Transcode((char*)dataTmp, padLen);
	char* ptrans = Encode(pEncode);
	free(pPadStr);
	free(pEncode);
	free(dataTmp);
