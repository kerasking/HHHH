...
// 		m_skirtSitImage = m_skirtLiftLegImage = colorInfoImage = NULL;
	...
	void NDSprite::SetCurrentAnimation(int animationIndex, bool reverse)
	{
		...
		if (m_aniGroup) 
		{
			if (animationIndex < 0 || animationIndex >= 0)//(int)m_aniGroup->getAnimations()->count()) ///< 临时性注释 郭浩
			{
				return;
			}
			...
			{
				m_currentAnimation = 0;//(NDAnimation*)m_aniGroup->getAnimations()->objectAtIndex(animationIndex); ///< 临时性注释 郭浩
...
//			m_currentAnimation->setReverse(m_reverse); ///< 临时性注释 郭浩
