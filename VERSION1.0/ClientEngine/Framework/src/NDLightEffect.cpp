...
namespace NDEngine
{
	...
	void NDLightEffect::Initialization(const char* sprFile)
	{
		...
		m_pkAniGroup = NDAnimationGroupPool::defaultPool()->
			addObjectWithSpr(sprFile);
	...
	void NDLightEffect::SlowDown(unsigned int mutli)
	{
		if (m_pkAniGroup)
		{
			NDAnimation *pkAnimation = (NDAnimation *)m_pkAniGroup->
				getAnimations()->objectAtIndex(m_nLightID);
	...
	{		
		...
		{	
			...
			NDAnimation *pkAnimation = (NDAnimation *)m_pkAniGroup->getAnimations()->objectAtIndex(m_nLightID);
