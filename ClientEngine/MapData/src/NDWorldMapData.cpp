...
NDWorldMapData * NDWorldMapData::SharedData()
{
	...
	{
		std::string mapFile = NDPath::GetMapPath().append + "map_99999.map";
...
{	
	...
	//------------------->节点
	m_PlaceNodes = new CCArray<PlaceNode*>();
	int nodeCount = op.readByte(stream);
	for (int i = 0; i < nodeCount; i++) 
	{
		PlaceNode *node = new PlaceNode;
		
		node.placeId = op.readShort(stream);
		...
		node.y->setY(op.readShort(stream));
...
PlaceNode * NDWorldMapData::getPlaceNodeWithMapId(int mapId)
{
	...
	{
		PlaceNode *node = m_PlaceNodes->objectAtIndex(i);
