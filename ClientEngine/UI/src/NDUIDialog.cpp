...
namespace NDEngine
{
	...
	void NDUIDialog::InitFullScrBtns(const std::vector<std::string>& ortherButtons)
	{
		...
		{
			float x = i < max_btns_half ? winsize.width-m_sizeBtnFullScr.width : 0,
				  y = m_uiTitleHeightFullScr + 13 + (i%max_btns_half)*(m_sizeBtnFullScr.height+4);
			...
			btn->Initialization();
			btn->SetFontSize(14);
			btn->SetFrameRect(CGRectMake(x, y, m_sizeBtnFullScr.width, m_sizeBtnFullScr.height));
			btn->SetDelegate(this);
			btn->SetFontColor(COLOR_OPTION_FULLSCR);
			btn->CloseFrame();
			btn->SetTouchDownImage(pool.AddPicture(GetImgPathNew("dlgfull_btn_click.png")), false, CGRectZero, true);
			this->AddChild(btn);
			
			if ((unsigned int)i < ortherButtons.size()) 
			{
				btn->SetTitle(ortherButtons[i].c_str(), false, false, leftWidth, rightWidth);
			}
			else 
			{
	...
	{
		if (m_lbTitle)	return m_lbTitle->GetText();
	...
	{		
		...
		sizeDialog.height	= m_bFullScreen ? winsize.height : m_sizeNotFullScr.height;
		...
		this->SetBackgroundImage(pool.AddPicture(GetImgPathNew(m_bFullScreen ? "dlgfull_bg.png" : "dlg_bg.png")), true);	
		...
		m_lbTime->SetFrameRect(CGRectMake(10, m_bFullScreen ? 14.5f : 18.0f, sizeDialog.width, sizeDialog.height));
		...
		m_lbTitle->SetFrameRect(CGRectMake(0, 0, sizeDialog.width, m_bFullScreen ? 29 : 36));
		...
		m_btnClose->SetImage(pool.AddPicture(GetImgPathNew(m_bFullScreen ? "dlgfull_close_normal.png" : "dlg_close_normal.png")), 
							false, CGRectZero, true);
		m_btnClose->SetTouchDownImage(pool.AddPicture(GetImgPathNew(m_bFullScreen ? "dlgfull_close_click.png" : "dlg_close_click.png")),
									  false, CGRectZero, true);
		...
		if (!m_bFullScreen)
		{
			...
			m_contentScroll->SetFrameRect(CGRectMake(18, 
													 m_uiTitleHeightNotFullScr, 
													 sizeDialog.width-18*2, 
													 m_uiContentHeightNotFullScr+(hasBtn ? 0 : m_sizeBtnNotFullScr.height+6)));
			...
			m_contentScroll->SetBackgroundImage(pool.AddPicture(GetImgPathNew("dlg_content.png"), 
			...
			if (hasBtn)
			{
				...
				m_btnConfirm->SetImage(pool.AddPicture(GetImgPathNew("dlg_btn_normal.png"), m_sizeBtnNotFullScr.width, m_sizeBtnNotFullScr.height), 
									   false, CGRectZero, true);
				m_btnConfirm->SetTouchDownImage(pool.AddPicture(GetImgPathNew("dlg_btn_click.png"), m_sizeBtnNotFullScr.width, m_sizeBtnNotFullScr.height), 
												false, CGRectZero, true);
				m_btnConfirm->SetFrameRect(CGRectMake(m_uiIntervalBtnAndContent+(sizeDialog.width-m_uiIntervalBtnAndContent*2-m_sizeBtnNotFullScr.width)/2, 
													  m_uiTitleHeightNotFullScr+m_contentScroll->GetFrameRect().size.height+6, 
		...
		else 
		{
			m_contentScroll = new NDUIContainerScrollLayer;
			m_contentScroll->Initialization();
			m_contentScroll->SetFrameRect(CGRectMake(m_uiIntervalBtnAndContent+m_sizeBtnFullScr.width, 
													 m_uiTitleHeightFullScr, 
													 sizeDialog.width-(m_uiIntervalBtnAndContent+m_sizeBtnFullScr.width)*2, 
	...
	void NDUIDialog::Show1(const char* title, const char* text, 
			   const char* op1)
	{
		this->Show(title, text, "",
				   op1 ? op1 : "",
	...
	void NDUIDialog::Show2(const char* title, const char* text, 
			   const char* op1,
			   const char* op2)
	{
		this->Show(title, text, "",
	...
	void NDUIDialog::Show3(const char* title, const char* text, 
			   const char* op1,
			   const char* op2,
			   const char* op3)
	{
	...
	void NDUIDialog::Show4(const char* title, const char* text, 
			   const char* op1,
			   const char* op2,
			   const char* op3,
			   const char* op4)
	...
	void NDUIDialog::Show5(const char* title, const char* text, 
			   const char* op1,
			   const char* op2,
			   const char* op3,
			   const char* op4,
	...
	void NDUIDialog::Show6(const char* title, const char* text, 
			   const char* op1,
			   const char* op2,
			   const char* op3,
			   const char* op4,
	...
	void NDUIDialog::Show7(const char* title, const char* text, 
			   const char* op1,
			   const char* op2,
			   const char* op3,
			   const char* op4,
	...
	void NDUIDialog::Show8(const char* title, const char* text, 
			   const char* op1,
			   const char* op2,
			   const char* op3,
			   const char* op4,
	...
	void NDUIDialog::Show9(const char* title, const char* text, 
			   const char* op1,
			   const char* op2,
			   const char* op3,
			   const char* op4,
	...
	void NDUIDialog::Show10(const char* title, const char* text, 
				const char* op1,
				const char* op2,
				const char* op3,
				const char* op4,
	...
	bool NDUIDialog::TouchBegin(NDTouch* touch)
	{
		...
		if (m_bFullScreen || CGRectContainsPoint(this->GetScreenRect(), m_beginTouch)) 
		{
	...
	bool NDUIDialog::OnLayerMove(NDUILayer* uiLayer, UILayerMove move, float distance)
	{
	...
	void NDUIDialog::SetContent(const char *text, ccColor4B color/*=ccc4(79, 79, 79, 255)*/, unsigned int fontsize/*=13*/)
	{
		...
		textSize.width = m_contentScroll->GetFrameRect().size.width 
						- (m_bFullScreen ? m_uiContentLeftWidthFullScr : m_uiContentLeftWidthNotFullScr)*2;
		
		textSize.height = NDUITextBuilder::DefaultBuilder()->StringHeightAfterFilter(text, textSize.width, fontsize);
		
		NDUIText* uiText = NDUITextBuilder::DefaultBuilder()->Build(text, 
																	fontsize, 
																	textSize, 
																	color,
																	false);
		
		uiText->SetFrameRect(CGRectMake((m_bFullScreen ? m_uiContentLeftWidthFullScr : m_uiContentLeftWidthNotFullScr),
										 m_uiContentTopheight, 
										 textSize.width,
										 textSize.height));
